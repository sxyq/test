...
V135_SPARSE_BATCHED_SMOE = os.environ.get("GRAB_V135_SPARSE_BATCHED_SMOE", "1").lower() not in {"0", "false", "no"}
V134_REMOVE_DEAD_FFN = os.environ.get("GRAB_V134_REMOVE_DEAD_FFN", "1").lower() not in {"0", "false", "no"}
...
class SMoE(nn.Module):
    ...
    def forward(self, x):
        ...
        if V135_SPARSE_BATCHED_SMOE and not self.training and self._v109_dense_weights_ready:
            # V135: Sparse Batched SMoE — only compute tokens routed to each expert
            # Dense SMoE computes all 8 experts for all tokens (75% wasted compute).
            # This path computes only the tokens actually routed to each expert,
            # using pre-stacked weights for efficient GEMM.
            # Total compute = 2/8 of Dense = 25% of Dense SMoE compute.
            x_flat = x.reshape(-1, D)  # [NS, D]
            NS = B * S
            out_flat = x_flat.new_zeros(NS, D)

            topk_exp = topk_idx.reshape(NS, self.k)  # [NS, 2]
            topk_sc = topk_score.reshape(NS, self.k)   # [NS, 2]

            # Per-expert batched computation
            for e in range(self.num_experts):
                mask = (topk_exp == e)  # [NS, 2]
                if not mask.any():
                    continue

                token_indices, k_indices = mask.nonzero(as_tuple=True)
                scores = topk_sc[token_indices, k_indices]  # [count]

                x_e = x_flat[token_indices]  # [count, D]

                # Use pre-stacked weights for efficient GEMM
                hidden = torch.matmul(x_e, self._all_fc1_w[e]) + self._all_fc1_b[e]
                hidden = F.relu(hidden)
                expert_out = torch.matmul(hidden, self._all_fc2_w[e]) + self._all_fc2_b[e]

                out_flat.index_add_(0, token_indices, expert_out * scores.unsqueeze(-1))
...
def apply_v134_remove_dead_ffn(model):
    """V134: Remove dead FFN layers (ffn1, ffn2) from TransformerEncoder.

    In the GRAB model, SMoE replaces FFN — ffn1/ffn2 are defined in __init__
    but NEVER called in forward(). They waste:
    - Memory: 8 layers × (512×1024 + 1024×512) × 2 bytes = ~8MB FP16
    - Checkpoint loading time: unnecessary weight loading
    - GPU memory: unnecessary parameter storage

    This is a structural cleanup — removing unused code, not changing
    the model's computation graph. The model produces identical outputs.
    """
    if not V134_REMOVE_DEAD_FFN:
        _info("[INFO] V134 remove dead FFN disabled by GRAB_V134_REMOVE_DEAD_FFN")
        return 0

    removed_params = 0
    for i in range(model.seq_encoder.num_layers):
        ffn1 = model.seq_encoder.ffn1[i]
        ffn2 = model.seq_encoder.ffn2[i]
        removed_params += sum(p.numel() for p in ffn1.parameters())
        removed_params += sum(p.numel() for p in ffn2.parameters())

    # Replace with empty ModuleList to free memory
    model.seq_encoder.ffn1 = nn.ModuleList()
    model.seq_encoder.ffn2 = nn.ModuleList()

    _info(f"[INFO] V134 removed dead FFN layers: freed {removed_params:,} parameters ({removed_params * 2 / 1024 / 1024:.1f} MB FP16)")
...
def load_model(ckpt_path=None, device='cuda:0'):
    ...
    # V134: Remove dead FFN layers AFTER weight loading but BEFORE .to(dev)
    # This frees GPU memory for the dead ffn1/ffn2 weights that SMoE replaces
    apply_v134_remove_dead_ffn(model)
