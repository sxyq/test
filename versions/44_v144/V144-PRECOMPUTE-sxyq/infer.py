...
def per_user_causal_sdpa(q, k, v, user_offsets_cpu, fa_max_seqlen=None, fa_cu_seqlens=None):
    ...
    Args:
        ...
        fa_max_seqlen: V144 pre-computed max seqlen to avoid per-layer GPU→CPU sync
        fa_cu_seqlens: V144 pre-computed cu_seqlens to avoid per-layer recompute
    ...
    if _FLASH_ATTN_AVAILABLE:
        ...
        # V144: Use pre-computed values if available, avoid per-layer GPU→CPU sync
        if fa_max_seqlen is not None and fa_cu_seqlens is not None:
            max_seqlen = fa_max_seqlen
            cu_seqlens = fa_cu_seqlens
        else:
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x, extension=None, user_offsets_cpu=None):
        ...
        # V144: Pre-compute flash_attn params ONCE to avoid 8x GPU→CPU sync per batch
        fa_max_seqlen = None
        fa_cu_seqlens = None
        if _FLASH_ATTN_AVAILABLE and V103_PER_USER_CAUSAL_SDPA and user_offsets_cpu is not None:
            fa_max_seqlen = int((user_offsets_cpu[1:] - user_offsets_cpu[:-1]).max())
            fa_cu_seqlens = user_offsets_cpu.to(torch.int32)
        ...
        for i in range(self.num_layers):
            ...
            if V103_PER_USER_CAUSAL_SDPA and user_offsets_cpu is not None:
                attn_out = per_user_causal_sdpa(q, k, v, user_offsets_cpu,
                                                fa_max_seqlen=fa_max_seqlen,
                                                fa_cu_seqlens=fa_cu_seqlens)
            else:
