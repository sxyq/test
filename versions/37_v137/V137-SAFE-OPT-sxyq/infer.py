...
def configure_runtime():
    ...
    try:
        ...
        pass
    # V137: Reduce CUDA allocator fragmentation for SMoE's frequent alloc/dealloc
    try:
...
def move_model_inputs_to_device(batch, device, non_blocking=False):
    moved = {
        # V137: user_offsets stays on CPU — avoids GPU→CPU sync in CTRModel.forward
...
class CTRModel(nn.Module):
    ...
    def forward(self, batch):
        ...
        if V103_PER_USER_CAUSAL_SDPA:
            # V137: user_offsets already on CPU, no .cpu() sync needed
