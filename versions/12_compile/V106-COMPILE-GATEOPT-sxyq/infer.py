...
V106_GATE_TOPK_ONLY = os.environ.get("GRAB_V106_GATE_TOPK_ONLY", "1").lower() not in {"0", "false", "no"}
V106_REDUCE_EMPTY_CACHE = os.environ.get("GRAB_V106_REDUCE_EMPTY_CACHE", "1").lower() not in {"0", "false", "no"}
...
class TopKGate(nn.Module):
    def __init__(self, d_model, num_experts, k=2, noisy_gating=True):
        ...
        self.v106_gate_topk_only = False
    ...
    def forward(self, x):
        ...
        # V106: 推理时跳过完整 softmax，只对 top-k logit 做 softmax
        if self.v106_gate_topk_only and not self.training:
            topk_raw, topk_idx = torch.topk(logits, self.k, dim=-1)  # [B,S,k]
            topk_score = torch.softmax(topk_raw, dim=-1)  # [B,S,k]
            return topk_idx, topk_score, None  # probs=None，推理时不需要
...
class SMoE(nn.Module):
    ...
    def forward(self, x):
        ...
        if V27_NO_AUXLOSS_ENABLED and not self.training:
            ...
            # probs 可能为 None（V106 gate topk-only 模式），此时跳过 aux loss
            if probs is None:
                moe_loss = x.new_zeros(())
            else:
...
class CTRModel(nn.Module):
    ...
    def forward(self, batch):
        ...
        if V103_PER_USER_CAUSAL_SDPA:
            # 保持与 V103 完全一致：user_offsets 先 .cpu() 再传入
...
def apply_v106_gate_topk_only(model):
    """Enable topk-only softmax in SMoE gate at inference time."""
    if not V106_GATE_TOPK_ONLY:
        _info("[INFO] V106 gate topk-only disabled by GRAB_V106_GATE_TOPK_ONLY")
        return 0
    converted = 0
    for moe in model.seq_encoder.moe:
        moe.gate.v106_gate_topk_only = True
        converted += 1
    _info(f"[INFO] V106 gate topk-only enabled: {converted} SMoE layers")
    return converted
...
def load_model(ckpt_path=None, device='cuda:0'):
    ...
    apply_v106_gate_topk_only(model)
    model.to(dev)
    model.eval()
    ...
    with torch.inference_mode(), autocast_ctx:
        if shard_files:
            ...
            for sf in shard_files:
                ...
                shard_idx += 1
                del shard_batches
                # V106: 减少 empty_cache 调用频率
                if use_cuda and V106_REDUCE_EMPTY_CACHE and shard_idx % 3 != 0:
                    pass
                elif use_cuda:
