...
# V130: Skip 4 middle layers + bias-shift PCOC calibration (ZERO extra latency)
# V129 proved: skip 3 layers + bias bake → PCOC=1.094, AUC=0.749, latency=37.93s
# V130 extrapolates: skip 4 layers → correction≈0.417, bias=log(0.417)≈-0.875
# PCOC trend: skip1=0.7658, skip2=0.6664, skip3=0.5269 → skip4≈0.417
# Risk: AUC may drop below 0.65, but worth trying for ~35s latency
V130_SKIP_LAYERS = os.environ.get("GRAB_V130_SKIP_LAYERS", "2,3,4,5")
V130_LOGIT_BIAS = float(os.environ.get("GRAB_V130_LOGIT_BIAS", str(math.log(0.417))))
