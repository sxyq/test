...
class ResultCollector:
    ...
    def add(self, logids, probs, pred_mask):
        if not self.enabled:
            self.all_logids.extend(logids[pred_mask].cpu().tolist())
            self.all_probs.extend(probs[pred_mask].cpu().tolist())
            return

        masked_logids = logids[pred_mask]
...
def main():
    ...
    def run_one_batch(batch):
        ...
        collector.add(batch["logid"], probs, pred_mask)
