...
V110_EINSUM_FUSION = os.environ.get("GRAB_V110_EINSUM_FUSION", "0").lower() not in {"0", "false", "no"}
...
V111_BMM_FUSION = os.environ.get("GRAB_V111_BMM_FUSION", "1").lower() not in {"0", "false", "no"}
...
class SMoE(nn.Module):
    ...
    def forward(self, x):
        ...
        if V109_DENSE_SMOE and not self.training and self._v109_dense_weights_ready:
            ...
            if V111_BMM_FUSION:
                # V111: Use bmm + addmm for fused GEMM+bias, more efficient than matmul + broadcast add
                # x_flat: [NS, D] → expand to [E, NS, D] for bmm
                x_exp = x_flat.unsqueeze(0).expand(self.num_experts, -1, -1)  # [E, NS, D]

                # Fused fc1: addmm(bias, input, weight) does GEMM+bias in one kernel
                # _all_fc1_w: [E, D, dim_ff], _all_fc1_b: [E, dim_ff]
                hidden = torch.baddbmm(
                    self._all_fc1_b.unsqueeze(1),  # [E, 1, dim_ff] broadcast to [E, NS, dim_ff]
                    x_exp,                          # [E, NS, D]
                    self._all_fc1_w,                # [E, D, dim_ff]
                )
                hidden = F.relu(hidden)

                # Fused fc2: same pattern
                out_all = torch.baddbmm(
                    self._all_fc2_b.unsqueeze(1),  # [E, 1, D]
                    hidden,                         # [E, NS, dim_ff]
                    self._all_fc2_w,                # [E, dim_ff, D]
                )
            else:
                # V109 original path: matmul + broadcast bias add
