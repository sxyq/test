...
# V110: Install flash-attn for varlen attention (single kernel, zero padding)
# This enables the fast path in per_user_causal_sdpa instead of Python loop fallback
echo "[V110] Attempting flash-attn installation..."
pip install flash-attn --no-build-isolation -i https://mirrors.aliyun.com/pypi/simple/ 2>/dev/null || \
    "${LIB_DIR}/bin/uv" pip install flash-attn --no-build-isolation --target "${LIB_DIR}" -i https://mirrors.aliyun.com/pypi/simple/ 2>/dev/null || \
    echo "[V110] flash-attn installation failed (will use SDPA fallback path)"
