...
# V118: Layer pruning + PCOC calibration
# V117 proved: skipping layer 4 reduces latency by 3.74s but PCOC shifts from 1.059→1.383
# V118 adds: probability scaling to correct PCOC back to ~1.059
...
V118_PCOC_CORRECTION = float(os.environ.get("GRAB_V118_PCOC_CORRECTION", "0.7658"))
...
def load_model(ckpt_path=None, device='cuda:0'):
    ...
    # V117/V118: Parse skip layers and configure
    skip_layers = []
    if V117_SKIP_LAYERS:
        try:
    ...
    if skip_layers:
        ...
        _info(f"[INFO] V118: skipping middle layers {skip_layers} (running {num_layers - len(skip_layers)}/{num_layers} layers)")

    if V118_PCOC_CORRECTION != 1.0:
        _info(f"[INFO] V118: PCOC correction factor = {V118_PCOC_CORRECTION} (probs *= {V118_PCOC_CORRECTION})")
