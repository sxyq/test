...
V82_SELECTED_OUTPUT_HEAD_ENABLED = os.environ.get("GRAB_V82_SELECTED_OUTPUT_HEAD", "1").lower() not in {"0", "false", "no"}
V82_SELECTED_HEAD_MAX_DENSITY = float(os.environ.get("GRAB_V82_SELECTED_HEAD_MAX_DENSITY", "0.10"))
V84_ZERO_PRED_BATCH_SKIP_ENABLED = os.environ.get("GRAB_V84_ZERO_PRED_BATCH_SKIP", "1").lower() not in {"0", "false", "no"}
V99_PRED_ROUTE_H2D_SKIP_ENABLED = os.environ.get("GRAB_V99_PRED_ROUTE_H2D_SKIP", "1").lower() not in {"0", "false", "no"}
V101_SINGLE_USER_CAUSAL_SDPA_ENABLED = os.environ.get("GRAB_V101_SINGLE_USER_CAUSAL_SDPA", "1").lower() not in {"0", "false", "no"}
...
class ResultCollector:
    ...
    def add_selected(self, logids, probs):
        if not self.enabled:
        ...
        if logids.numel() == 0:
            return
        self.logid_parts.append(logids)
        self.prob_parts.append(probs)
        self.pending += int(logids.numel())
        if self.pending >= self.chunk_preds:
            self.flush()
...
class CTRModel(nn.Module):
    ...
    def forward(self, batch):
        ...
        single_user_causal = (
            V101_SINGLE_USER_CAUSAL_SDPA_ENABLED
            and batch["user_offsets"].numel() == 2
        )
        if single_user_causal:
            extension = {"is_causal": True, "v101_single_user_causal_active": True}
        else:
        ...
        pred_indices = batch.get("v82_pred_indices")
        if V82_SELECTED_OUTPUT_HEAD_ENABLED and pred_indices is not None:
            pred_indices = pred_indices.to(device=encoder_output.device, dtype=torch.long)
            if pred_indices.numel() == 0:
                pred_logits = encoder_output.new_empty((0, 1))
            else:
                encoder_output = encoder_output.index_select(0, pred_indices)
...
def main():
    ...
    def run_one_batch(batch):
        ...
        if V30_CPU_METADATA_ENABLED:
            ...
            if (
                V99_PRED_ROUTE_H2D_SKIP_ENABLED
                and V84_ZERO_PRED_BATCH_SKIP_ENABLED
                and pred_count == 0
            ):
                return
            ...
            if (
                not V99_PRED_ROUTE_H2D_SKIP_ENABLED
                and V84_ZERO_PRED_BATCH_SKIP_ENABLED
                and pred_count == 0
            ):
                return
            ...
            if use_pred_indices:
                ...
                pred_density = (pred_count / pred_total) if pred_total > 0 else 0.0
                selected_head_active = (
                    V82_SELECTED_OUTPUT_HEAD_ENABLED
                    and (pred_count == 0 or pred_density < V82_SELECTED_HEAD_MAX_DENSITY)
                )
            else:
        ...
        t_start = time.time()
        if selected_head_active:
            model_batch["v82_pred_indices"] = pred_indices_device
        ...
        if selected_head_active:
            collector.add_selected(cpu_logids.index_select(0, pred_indices_cpu), probs)
        elif pred_indices_cpu is not None and V30_CPU_METADATA_ENABLED:
