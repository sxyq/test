...
V149_CUDA_GRAPH = os.environ.get("GRAB_V149_CUDA_GRAPH", "1").lower() not in {"0", "false", "no"}
...
class SMoE(nn.Module):
    def __init__(self, d_model, dim_ff, num_experts, k=2):
        ...
        self._v149_graph = None
        self._v149_max_ns = int(os.environ.get("GRAB_V149_MAX_NS", "5000"))
    ...
    def _v109_prepare_dense_weights(self):
        ...
        # V149: Capture CUDA Graph for SMoE expert GEMMs
        # Eliminates kernel launch overhead for the 2 batched GEMMs + weighted sum
        # Padding tokens get zero gate weights → don't affect output
        if V149_CUDA_GRAPH and not self.training and self._v147_int8_active == False:
            try:
                device = self._all_fc1_w.device
                dtype = self._all_fc1_w.dtype
                E = self.num_experts
                D_w = self._all_fc1_w.shape[1]
                max_ns = self._v149_max_ns

                # Pre-allocate static input buffers
                self._v149_static_x = torch.zeros(1, max_ns, D_w, device=device, dtype=dtype)
                self._v149_static_gw = torch.zeros(max_ns, E, device=device, dtype=dtype)

                # Warm up (let cuBLAS select best algorithms)
                for _ in range(3):
                    h = torch.matmul(self._v149_static_x, self._all_fc1_w) + self._all_fc1_b.unsqueeze(1)
                    h = F.relu(h)
                    oa = torch.matmul(h, self._all_fc2_w) + self._all_fc2_b.unsqueeze(1)
                    _ = (oa * self._v149_static_gw.t().unsqueeze(-1)).sum(dim=0)
                torch.cuda.synchronize()

                # Capture graph
                self._v149_graph = torch.cuda.CUDAGraph()
                with torch.cuda.graph(self._v149_graph):
                    h = torch.matmul(self._v149_static_x, self._all_fc1_w) + self._all_fc1_b.unsqueeze(1)
                    h = F.relu(h)
                    oa = torch.matmul(h, self._all_fc2_w) + self._all_fc2_b.unsqueeze(1)
                    self._v149_static_out = (oa * self._v149_static_gw.t().unsqueeze(-1)).sum(dim=0)

                _info(f"[INFO] V149: CUDA Graph captured for SMoE (max_ns={max_ns})")
            except Exception as e:
                _warn(f"[WARNING] V149 CUDA Graph capture failed: {e}, using eager mode")
                self._v149_graph = None
    ...
    def forward(self, x):
        ...
        if V109_DENSE_SMOE and not self.training and self._v109_dense_weights_ready:
            ...
            # V149: CUDA Graph replay path (eliminates kernel launch overhead)
            if self._v149_graph is not None and NS <= self._v149_max_ns:
                # Copy actual data into static buffers
                self._v149_static_x[0, :NS, :].copy_(x_flat)
                self._v149_static_gw[:NS, :].copy_(gate_weights)
                # Zero padding (padding tokens get zero gate weights → don't affect output)
                if NS < self._v149_max_ns:
                    self._v149_static_x[0, NS:, :].zero_()
                    self._v149_static_gw[NS:, :].zero_()
                # Replay graph (single launch replaces 5+ kernel launches)
                self._v149_graph.replay()
                # Read output
                out = self._v149_static_out[:NS, :].reshape(B, S, D)
            elif self._v147_int8_active:
