...
def configure_runtime():
    ...
    try:
        ...
        pass
    # V136: Reduce CUDA allocator fragmentation for SMoE's frequent alloc/dealloc
    try:
...
def move_model_inputs_to_device(batch, device, non_blocking=False):
    moved = {
        # V136: user_offsets stays on CPU — avoids GPU→CPU sync in CTRModel.forward
...
class TopKGate(nn.Module):
    ...
    def forward(self, x):
        ...
        # V136: Inference — topk on logits directly, softmax only on top-k
        if not self.training:
            topk_score, topk_idx = torch.topk(logits, self.k, dim=-1)  # [B,S,k]
            topk_score = torch.softmax(topk_score, dim=-1)  # only normalize top-k
            return topk_idx, topk_score, None  # probs not needed in inference

        # Training — full softmax for aux loss
...
class SMoE(nn.Module):
    ...
    def forward(self, x):
        ...
        if V109_DENSE_SMOE and not self.training and self._v109_dense_weights_ready:
            # V136: Dense batched SMoE with fused operations
            ...
            # V136 P1-1: baddbmm fuses GEMM + bias into single kernel
            # fc1: [E, NS, D] @ [E, D, dim_ff] + [E, 1, dim_ff] → [E, NS, dim_ff]
            x_exp = x_flat.unsqueeze(0).expand(self.num_experts, -1, -1)
            hidden = torch.baddbmm(
                self._all_fc1_b.unsqueeze(1),  # [E, 1, dim_ff] broadcast
                x_exp,                          # [E, NS, D]
                self._all_fc1_w,                # [E, D, dim_ff]
            )
            hidden = F.relu(hidden)
            # fc2: [E, NS, dim_ff] @ [E, dim_ff, D] + [E, 1, D] → [E, NS, D]
            out_all = torch.baddbmm(
                self._all_fc2_b.unsqueeze(1),  # [E, 1, D] broadcast
                hidden,                         # [E, NS, dim_ff]
                self._all_fc2_w,                # [E, dim_ff, D]
            )

            # V136 P0-3: einsum fuses weighted sum — avoids [E, NS, D] intermediate
            # gate_weights.t(): [E, NS], out_all: [E, NS, D]
        ...
        if V27_NO_AUXLOSS_ENABLED and not self.training:
            ...
            # V136: probs is None during inference (gate returns None)
            moe_loss = x.new_zeros(())
        else:
...
class TransformerEncoder(nn.Module):
    def __init__(self, d_model, n_heads, num_layers, dim_ff, act="relu",
        ...
        # V136: ffn1/ffn2 are dead code — SMoE replaces FFN, these are never called.
        # Kept as empty ModuleLists to avoid state_dict key mismatch (filtered in load_model).
...
class CTRModel(nn.Module):
    ...
    def forward(self, batch):
        ...
        if V103_PER_USER_CAUSAL_SDPA:
            # V136: user_offsets already on CPU, no .cpu() sync needed
            user_offsets_cpu = batch["user_offsets"]
...
def load_model(ckpt_path=None, device='cuda:0'):
    ...
    if ckpt_path.exists():
        ...
        # V136: Filter out dead FFN keys before load_state_dict
        # ffn1/ffn2 are never used (SMoE replaces FFN), removing them saves
        # ~34MB GPU memory and checkpoint loading time
        state_dict = {k: v for k, v in state_dict.items()
                      if not k.startswith('seq_encoder.ffn1.') and not k.startswith('seq_encoder.ffn2.')}
