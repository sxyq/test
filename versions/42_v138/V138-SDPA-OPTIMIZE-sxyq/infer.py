...
V134_MEMORY_OPTIMIZE = os.environ.get("GRAB_V134_MEMORY_OPTIMIZE", "1").lower() not in {"0", "false", "no"}
V135_CUDA_GRAPHS = os.environ.get("GRAB_V135_CUDA_GRAPHS", "1").lower() not in {"0", "false", "no"}
V136_INT8_QUANTIZE = os.environ.get("GRAB_V136_INT8_QUANTIZE", "1").lower() not in {"0", "false", "no"}
V137_DATA_PIPELINE = os.environ.get("GRAB_V137_DATA_PIPELINE", "1").lower() not in {"0", "false", "no"}
...
def configure_runtime():
    ...
    # V134: 内存优化
    if V134_MEMORY_OPTIMIZE:
        try:
            torch.cuda.memory._set_allocator_settings("expandable_segments:True")
            _info("[INFO] V134: enabled expandable segments for memory optimization")
        except Exception as e:
            _info(f"[INFO] V134: expandable segments not available: {e}")
...
def load_model(ckpt_path=None, device='cuda:0'):
    ...
    # V136: INT8动态量化（对Transformer中的Linear层）
    if V136_INT8_QUANTIZE and dev.type == "cuda":
        try:
            # 收集需要量化的Linear层类型
            layers_to_quantize = set()
            for i in range(model.seq_encoder.num_layers):
                layers_to_quantize.add(type(model.seq_encoder.qkv_proj[i]))
                layers_to_quantize.add(type(model.seq_encoder.out_proj[i]))
                layers_to_quantize.add(type(model.seq_encoder.moe[i].gate))
            layers_to_quantize.add(type(model.linear))
            ...
            _info("[INFO] V136: INT8 dynamic quantization applied to Linear layers")
        except Exception as e:
            _warn(f"[WARNING] V136: INT8 quantization failed: {e}, using FP16 only")
...
def main():
    ...
    if batches_cache_dir.exists() and any(batches_cache_dir.glob('shard_*.pt')):
        ...
        test_loader = DataLoader(
            ...
            batch_size=50,
            shuffle=False,
            num_workers=2 if V137_DATA_PIPELINE else 0,
            collate_fn=make_collate_fn(test_dataset.max_slot_id),
            pin_memory=V137_DATA_PIPELINE,
            prefetch_factor=2 if V137_DATA_PIPELINE else None,
        )
    ...
    def run_model_with_graph(model_batch):
        """使用CUDA Graphs加速模型前向传播"""
        if not (V135_CUDA_GRAPHS and use_cuda):
            return model(model_batch)
        
        # 获取batch的静态shape作为key
        batch_key = tuple(
            (k, tuple(v.shape) if torch.is_tensor(v) else type(v))
            for k, v in sorted(model_batch.items())
        )
        
        if batch_key not in cuda_graph_cache:
            # 第一次运行：捕获graph
            _info(f"[INFO] V135: capturing CUDA graph for batch shape")
            
            # 创建静态输入tensor（用于graph捕获和重放）
            static_inputs = {}
            ...
            with torch.cuda.graph(g):
                static_out = model(static_inputs)
            
            cuda_graph_cache[batch_key] = (g, static_inputs, static_out)
            _info(f"[INFO] V135: captured graph successfully")
        
        # 复制实际数据到静态输入tensor
        g, static_inputs, static_out = cuda_graph_cache[batch_key]
        for k, v in model_batch.items():
            if torch.is_tensor(v) and k in static_inputs:
    ...
    def run_one_batch(batch):
        ...
        t_start = time.time()
        model_out = run_model_with_graph(model_batch)
