...
# V127: Skip 3 middle layers + exact PCOC calibration (sigmoid→scale→logit)
# V122 proved: skip 2 layers + bias bake → PCOC=1.077, latency=39.71s, score=69.64
# V127: skip 3 layers for more latency reduction, use exact correction for reliable PCOC
# PCOC estimation: 0 layers=1.059, 1 layer=1.383, 2 layers≈1.617
#   3 layers estimated ~1.85 (linear extrapolation)
# correction = 1.059/1.85 ≈ 0.5724
V127_SKIP_LAYERS = os.environ.get("GRAB_V127_SKIP_LAYERS", "3,4,5")
V127_PCOC_CORRECTION = float(os.environ.get("GRAB_V127_PCOC_CORRECTION", "0.5724"))
...
class CTRModel(nn.Module):
    ...
    def forward(self, batch):
        ...
        # V127: PCOC calibration at MODEL OUTPUT level (same as V119, exact method)
        ...
        if not self.training and hasattr(self, '_v127_pcoc_correction') and self._v127_pcoc_correction != 1.0:
            probs = torch.sigmoid(pred_logits)
            scaled_probs = probs * self._v127_pcoc_correction
...
def load_model(ckpt_path=None, device='cuda:0'):
    ...
    # V127: Skip 2 middle layers
    skip_layers = []
    if V127_SKIP_LAYERS:
        try:
            skip_layers = [int(x.strip()) for x in V127_SKIP_LAYERS.split(',') if x.strip()]
        except ValueError:
            _warn(f"[WARNING] Invalid V127_SKIP_LAYERS: {V127_SKIP_LAYERS}")
            skip_layers = []
    if skip_layers:
        skip_layers = [i for i in skip_layers if 0 < i < num_layers - 1]
        model.seq_encoder.set_skip_layers(skip_layers)
        _info(f"[INFO] V127: skipping middle layers {skip_layers} (running {num_layers - len(skip_layers)}/{num_layers} layers)")

    # V127: Set PCOC correction factor for exact model-level correction
    if V127_PCOC_CORRECTION != 1.0:
        model._v127_pcoc_correction = V127_PCOC_CORRECTION
        _info(f"[INFO] V127: PCOC correction factor = {V127_PCOC_CORRECTION} (exact method, adds ~5s latency)")
