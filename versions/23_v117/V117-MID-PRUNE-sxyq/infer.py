...
# V117: Skip specific middle layers during inference (comma-separated 0-indexed layer indices)
# Default: skip layer 4 (5th of 8, middle layer where redundancy is typically highest)
# V116 proved: skipping the LAST layer causes PCOC collapse (1.06→2.35)
# Middle layers are safer to prune while preserving output calibration
...
class TransformerEncoder(nn.Module):
    def __init__(self, d_model, n_heads, num_layers, dim_ff, act="relu",
        ...
        # V117: set of layer indices to skip during inference
        self._skip_layer_indices = set()

    def set_skip_layers(self, indices):
        """Set which layer indices to skip during inference."""
    ...
    def forward(self, x, extension=None, user_offsets_cpu=None):
        ...
        for i in range(self.num_layers):
            # V117: Skip specified middle layers during inference
            # The residual connection naturally handles skipped layers:
            # x stays unchanged, equivalent to identity for that layer
...
def load_model(ckpt_path=None, device='cuda:0'):
    ...
    if V117_SKIP_LAYERS:
        try:
            ...
            skip_layers = []
    if skip_layers:
        # Safety: never skip first or last layer
        skip_layers = [i for i in skip_layers if 0 < i < num_layers - 1]
        model.seq_encoder.set_skip_layers(skip_layers)
        _info(f"[INFO] V117: skipping middle layers {skip_layers} (running {num_layers - len(skip_layers)}/{num_layers} layers)")
