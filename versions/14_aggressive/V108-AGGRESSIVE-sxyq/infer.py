...
V108_SMOE_SYNC_FREE = os.environ.get("GRAB_V108_SMOE_SYNC_FREE", "1").lower() not in {"0", "false", "no"}
V108_EXPERT_JIT = os.environ.get("GRAB_V108_EXPERT_JIT", "1").lower() not in {"0", "false", "no"}
V108_OFFSETS_STAY_CPU = os.environ.get("GRAB_V108_OFFSETS_STAY_CPU", "1").lower() not in {"0", "false", "no"}
V108_DYNAMIC_QUANT = os.environ.get("GRAB_V108_DYNAMIC_QUANT", "0").lower() not in {"0", "false", "no"}
...
def move_model_inputs_to_device(batch, device, non_blocking=False):
    moved = {}
    # V108: user_offsets 留在 CPU，避免 GPU→CPU 往返
    if V108_OFFSETS_STAY_CPU and V103_PER_USER_CAUSAL_SDPA:
        moved["user_offsets"] = batch["user_offsets"]  # 保留在 CPU
    else:
...
class SMoE(nn.Module):
    ...
    def forward(self, x):
        ...
        if V108_SMOE_SYNC_FREE and not self.training:
            # V108: 纯 tensor 操作，消除 96 次 .item() GPU→CPU 同步
            ...
            for i in range(self.num_experts):
                # 用 tensor 切片替代 .item()，避免 GPU→CPU 同步
                start_i = starts[i]
                count_i = counts[i]
                end_i = start_i + count_i
                # 当 count_i == 0 时，start_i == end_i，切片为空，index_select 返回空 tensor
                cur_token_idx = token_idx_sorted[start_i:end_i]
                cur_weight = expert_score_sorted[start_i:end_i]
                if cur_token_idx.numel() > 0:
...
class CTRModel(nn.Module):
    ...
    def forward(self, batch):
        ...
        if V103_PER_USER_CAUSAL_SDPA:
            # V108: user_offsets 可能已在 CPU（V108_OFFSETS_STAY_CPU），避免冗余 .cpu()
            user_offsets = batch["user_offsets"]
            user_offsets_cpu = user_offsets if user_offsets.device.type == "cpu" else user_offsets.cpu()
...
def apply_v108_expert_jit(model):
    """Apply torch.jit.script to Expert MLP for operator fusion."""
    if not V108_EXPERT_JIT:
        _info("[INFO] V108 Expert JIT disabled by GRAB_V108_EXPERT_JIT")
        return 0
    converted = 0
    for moe in model.seq_encoder.moe:
        for i, expert in enumerate(moe.experts):
            try:
                scripted = torch.jit.script(expert)
                moe.experts[i] = scripted
                converted += 1
            except Exception as e:
                _warn(f"[WARNING] JIT script failed for expert {i}: {e}")
    _info(f"[INFO] V108 Expert JIT: {converted}/64 experts scripted")
    return converted


def apply_v108_dynamic_quant(model):
    """Apply PyTorch dynamic quantization to Expert Linear layers.

    WARNING: Conflicts with V14 FP16 weights. Only enable if V14 is disabled.
    Default OFF (GRAB_V108_DYNAMIC_QUANT=0).
    """
    if not V108_DYNAMIC_QUANT:
        _info("[INFO] V108 dynamic quantization disabled (default OFF, conflicts with FP16)")
        return 0
    if V14_FP16_WEIGHTS_ENABLED:
        _warn("[WARNING] V108 dynamic quantization conflicts with V14 FP16, skipping")
        return 0
    try:
        model.seq_encoder = torch.ao.quantization.quantize_dynamic(
            model.seq_encoder,
            {nn.Linear},
            dtype=torch.qint8,
        )
        _info("[INFO] V108 dynamic quantization applied to seq_encoder")
        return 1
    except Exception as e:
        _warn(f"[WARNING] V108 dynamic quantization failed: {e}, skipping")
        return 0
...
def load_model(ckpt_path=None, device='cuda:0'):
    ...
    apply_v108_expert_jit(model)
    model.to(dev)
    model.eval()
    # 动态量化必须在 .to(dev) + .eval() 之后
    apply_v108_dynamic_quant(model)
    ...
    with torch.inference_mode(), autocast_ctx:
        if shard_files:
            ...
            for sf in shard_files:
                ...
                del shard_batches
                shard_idx += 1
                # V108: 每 3 个 shard 才 empty_cache，减少 CUDA 同步
