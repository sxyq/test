...
def per_user_causal_sdpa(q, k, v, user_offsets_cpu, fa_params=None):
    ...
    Args:
        ...
        fa_params: optional pre-computed (cu_seqlens, max_seqlen) tuple
    ...
    if _FLASH_ATTN_AVAILABLE:
        ...
        if fa_params is not None:
            cu_seqlens, max_seqlen = fa_params
        else:
...
class SMoE(nn.Module):
    ...
    def forward(self, x):
        ...
        if V109_DENSE_SMOE and not self.training and self._v109_dense_weights_ready:
            ...
            # V141: baddbmm fuses matmul+bias into one kernel
            # [E, NS, dim_ff] = bmm([E, NS, D], [E, D, dim_ff]) + [E, 1, dim_ff]
            hidden = torch.baddbmm(
                self._all_fc1_b.unsqueeze(1),
                x_flat.unsqueeze(0).expand(self.num_experts, -1, -1),
                self._all_fc1_w,
            )
            hidden = F.relu(hidden)
            # [E, NS, D] = bmm([E, NS, dim_ff], [E, dim_ff, D]) + [E, 1, D]
            out_all = torch.baddbmm(
                self._all_fc2_b.unsqueeze(1),
                hidden,
                self._all_fc2_w,
            )

            # Weighted sum: einsum fuses multiply+sum into one kernel
            # out_all: [E, NS, D], gate_weights.t(): [E, NS] → out: [NS, D]
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x, extension=None, user_offsets_cpu=None):
        ...
        # V141: Pre-compute flash_attn params once per forward (reused across all 8 layers)
        fa_params = None
        if V103_PER_USER_CAUSAL_SDPA and user_offsets_cpu is not None and _FLASH_ATTN_AVAILABLE:
            max_seqlen = int((user_offsets_cpu[1:] - user_offsets_cpu[:-1]).max())
            cu_seqlens = user_offsets_cpu.to(torch.int32)
            fa_params = (cu_seqlens, max_seqlen)
        ...
        for i in range(self.num_layers):
            ...
            if V103_PER_USER_CAUSAL_SDPA and user_offsets_cpu is not None:
                attn_out = per_user_causal_sdpa(q, k, v, user_offsets_cpu, fa_params=fa_params)
            else:
