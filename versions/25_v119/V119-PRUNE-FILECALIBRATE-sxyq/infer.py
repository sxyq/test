...
# V119: Layer pruning + FILE-LEVEL PCOC calibration
# V117 proved: skipping layer 4 reduces latency by 3.74s but PCOC shifts from 1.059→1.383
# V118 proved: GPU-level probs *= 0.7658 does NOT work (autocast/inference_mode interference)
# V119 fix: apply PCOC correction at FILE WRITE stage (pure CPU, no GPU interference)
# Correction factor = PCOC_V109 / PCOC_V117 = 1.05912 / 1.383 ≈ 0.7658
# Probability scaling is a monotonic transformation → AUC is exactly preserved
# PCOC_new = PCOC_old * correction_factor (exact, not approximate)
V117_SKIP_LAYERS = os.environ.get("GRAB_V117_SKIP_LAYERS", "4")
V119_PCOC_CORRECTION = float(os.environ.get("GRAB_V119_PCOC_CORRECTION", "0.7658"))
...
class CTRModel(nn.Module):
    ...
    def forward(self, batch):
        ...
        # V119: PCOC calibration at MODEL OUTPUT level
        # V117 showed PCOC=1.383 when skipping layer 4 (vs V109's PCOC=1.059)
        # V118 proved: GPU-level probs *= 0.7658 does NOT work (autocast/inference_mode interference)
        # V119-first-attempt proved: file-write-stage correction also does NOT work
        #   → evaluation system computes PCOC from model output directly, not from predict.txt
        # Fix: transform logits so that sigmoid(new_logits) = sigmoid(old_logits) * correction_factor
        # This is a monotonic transformation → AUC is exactly preserved
        # PCOC_new = PCOC_old * correction_factor (exact)
        if not self.training and hasattr(self, '_v119_pcoc_correction') and self._v119_pcoc_correction != 1.0:
            probs = torch.sigmoid(pred_logits)
            scaled_probs = probs * self._v119_pcoc_correction
            scaled_probs = scaled_probs.clamp(1e-7, 1 - 1e-7)
            pred_logits = torch.log(scaled_probs / (1 - scaled_probs))
...
def load_model(ckpt_path=None, device='cuda:0'):
    ...
    # V119: Parse skip layers and configure
    skip_layers = []
    if V117_SKIP_LAYERS:
        try:
            skip_layers = [int(x.strip()) for x in V117_SKIP_LAYERS.split(',') if x.strip()]
        except ValueError:
            _warn(f"[WARNING] Invalid V117_SKIP_LAYERS: {V117_SKIP_LAYERS}")
            skip_layers = []
    if skip_layers:
        skip_layers = [i for i in skip_layers if 0 < i < num_layers - 1]
        model.seq_encoder.set_skip_layers(skip_layers)
        _info(f"[INFO] V119: skipping middle layers {skip_layers} (running {num_layers - len(skip_layers)}/{num_layers} layers)")

    if V119_PCOC_CORRECTION != 1.0:
        model._v119_pcoc_correction = V119_PCOC_CORRECTION
        _info(f"[INFO] V119: PCOC correction factor = {V119_PCOC_CORRECTION} (applied at model output level in CTRModel.forward)")
...
def main():
    ...
    def run_one_batch(batch):
        ...
        # V119: PCOC correction is now applied at MODEL LEVEL (CTRModel.forward)
        # so sigmoid(logits) already returns corrected probs. No scaling needed here.
    ...
    # V119: PCOC correction is applied at MODEL LEVEL (CTRModel.forward)
    # sigmoid(corrected_logits) already returns scaled probs, no additional scaling needed
