...
V113_COMPILE_SMOE = os.environ.get("GRAB_V113_COMPILE_SMOE", "1").lower() not in {"0", "false", "no"}
V113_COMPILE_REP = os.environ.get("GRAB_V113_COMPILE_REP", "1").lower() not in {"0", "false", "no"}
...
def load_model(ckpt_path=None, device='cuda:0'):
    ...
    # V113: torch.compile with dynamic=True — only compile SMoE modules (pure tensor ops, most stable)
    # V112 failed catastrophically with reduce-overhead (CUDA Graphs + dynamic shapes = recompile every batch)
    if V113_COMPILE_SMOE:
        try:
            _info("[INFO] V113: Compiling SMoE modules with dynamic=True, mode=default...")
            for i, moe in enumerate(model.seq_encoder.moe):
                model.seq_encoder.moe[i] = torch.compile(moe, mode="default", dynamic=True)
            _info("[INFO] V113: SMoE modules compiled successfully")
        except Exception as e:
            _info(f"[INFO] V113: SMoE compile failed ({e}), falling back to eager")

    if V113_COMPILE_REP:
        try:
            _info("[INFO] V113: Compiling rep_encoder with dynamic=True, mode=default...")
            model.rep_encoder = torch.compile(model.rep_encoder, mode="default", dynamic=True)
            _info("[INFO] V113: rep_encoder compiled successfully")
        except Exception as e:
            _info(f"[INFO] V113: rep_encoder compile failed ({e}), falling back to eager")
...
def main():
    ...
    with torch.inference_mode(), autocast_ctx:
        # V113: Warmup for torch.compile — trigger JIT compilation before timing starts
        if (V113_COMPILE_SMOE or V113_COMPILE_REP) and shard_files:
            _info("[INFO] V113: Running compile warmup...")
            ...
            if len(warmup_shard) > 0:
                ...
                _info("[INFO] V113: Compile warmup done")
            del warmup_shard
