...
V112_COMPILE = os.environ.get("GRAB_V112_COMPILE", "1").lower() not in {"0", "false", "no"}
V112_COMPILE_MODE = os.environ.get("GRAB_V112_COMPILE_MODE", "reduce-overhead")
V112_COMPILE_REP_ENCODER = os.environ.get("GRAB_V112_COMPILE_REP_ENCODER", "1").lower() not in {"0", "false", "no"}
...
def load_model(ckpt_path=None, device='cuda:0'):
    ...
    # V112: torch.compile for operator fusion and Python overhead elimination
    # V109's dense SMoE is pure tensor ops — ideal for torch.compile
    if V112_COMPILE:
        try:
            _info(f"[INFO] V112: Compiling seq_encoder with mode={V112_COMPILE_MODE}...")
            model.seq_encoder = torch.compile(model.seq_encoder, mode=V112_COMPILE_MODE)
            _info("[INFO] V112: seq_encoder compiled successfully")
        except Exception as e:
            _info(f"[INFO] V112: torch.compile failed ({e}), falling back to eager")
            # If compile fails, the model still works in eager mode

        if V112_COMPILE_REP_ENCODER:
            try:
                _info(f"[INFO] V112: Compiling rep_encoder with mode={V112_COMPILE_MODE}...")
                model.rep_encoder = torch.compile(model.rep_encoder, mode=V112_COMPILE_MODE)
                _info("[INFO] V112: rep_encoder compiled successfully")
            except Exception as e:
                _info(f"[INFO] V112: rep_encoder compile failed ({e}), falling back to eager")
...
def main():
    ...
    with torch.inference_mode(), autocast_ctx:
        # V112: Warmup for torch.compile — trigger JIT compilation before timing starts
        if V112_COMPILE and shard_files:
            _info("[INFO] V112: Running compile warmup...")
            warmup_shard = torch.load(shard_files[0], map_location='cpu', weights_only=False)
            if len(warmup_shard) > 0:
                warmup_batch = move_model_inputs_to_device(warmup_shard[0], dev, non_blocking=use_cuda)
                _ = model(warmup_batch)
                torch.cuda.synchronize() if use_cuda else None
                _info("[INFO] V112: Compile warmup done")
            del warmup_shard
