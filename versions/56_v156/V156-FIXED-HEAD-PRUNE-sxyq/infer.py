...
V155_PRUNE_HEAD_DIM = os.environ.get("GRAB_V155_PRUNE_HEAD_DIM", "1").lower() not in {"0", "false", "no"}
...
def load_model(ckpt_path=None, device='cuda:0'):
    ...
    if V155_PRUNE_HEAD_DIM:
        try:
            ...
            if new_head_dim < old_head_dim and new_head_dim > 0:
                with torch.no_grad():
                    for i in range(seq_enc.num_layers):
                        ...
                        # Layout: output features are [n_heads, 3*head_dim] (head-major)
                        qkv_w = qkv.weight.data.contiguous().view(n_heads, 3, old_head_dim, d_model)
                        # OutProj weight: [d_model, d_model] → [d_model, n_heads, head_dim]
                        out_w = outp.weight.data.contiguous().view(d_model, n_heads, old_head_dim)
                        ...
                        qkv.bias.data = qkv.bias.data.contiguous().view(n_heads, 3, old_head_dim)[:, :, keep].contiguous().view(-1)
            ...
            import traceback
            _warn(f"[WARNING] V155 head_dim pruning failed: {e}\n{traceback.format_exc()}")
            _warn("[WARNING] Using full head_dim (V155 head pruning disabled)")
