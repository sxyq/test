...
# V116: Number of Transformer layers to prune from the end (0=none, 1=skip last layer, etc.)
V116_PRUNE_LAYERS = int(os.environ.get("GRAB_V116_PRUNE_LAYERS", "1"))
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x, extension=None, user_offsets_cpu=None):
        ...
        # V116: Layer pruning — skip the last N layers during inference
        layers_to_run = self.num_layers
        if not self.training and V116_PRUNE_LAYERS > 0:
            layers_to_run = self.num_layers - V116_PRUNE_LAYERS
            if layers_to_run < 1:
                layers_to_run = 1
...
def load_model(ckpt_path=None, device='cuda:0'):
    ...
    if V116_PRUNE_LAYERS > 0:
        _info(f"[INFO] V116 layer pruning: running {num_layers - V116_PRUNE_LAYERS}/{num_layers} Transformer layers")
