...
V115_BUFFER_SMOE = os.environ.get("GRAB_V115_BUFFER_SMOE", "1").lower() not in {"0", "false", "no"}
V115_NO_AUTOCAST = os.environ.get("GRAB_V115_NO_AUTOCAST", "1").lower() not in {"0", "false", "no"}
V115_GATHER_TOPK = os.environ.get("GRAB_V115_GATHER_TOPK", "1").lower() not in {"0", "false", "no"}
...
class SMoE(nn.Module):
    def __init__(self, d_model, dim_ff, num_experts, k=2):
        ...
        # V115: pre-allocated buffer fields
        self._buf_hidden = None
        self._buf_out_all = None
        self._buf_ns = 0
    ...
    def _v115_ensure_buffers(self, ns):
        """V115: Ensure pre-allocated buffers are large enough for ns tokens.
        Buffers are allocated once and reused across all forward calls.
        """
        if not V115_BUFFER_SMOE:
            return
        if self._buf_hidden is not None and self._buf_ns >= ns:
            return
        # Allocate with 25% headroom to reduce re-allocation frequency
        alloc_ns = int(ns * 1.25) + 64
        dtype = self._all_fc1_w.dtype
        device = self._all_fc1_w.device
        self._buf_hidden = torch.empty(
            self.num_experts, alloc_ns, self.dim_ff,
            dtype=dtype, device=device,
        )
        self._buf_out_all = torch.empty(
            self.num_experts, alloc_ns, self.d_model,
            dtype=dtype, device=device,
        )
        self._buf_ns = alloc_ns
    ...
    def forward(self, x):
        ...
        if V109_DENSE_SMOE and not self.training and self._v109_dense_weights_ready:
            ...
            if V115_BUFFER_SMOE:
                # V115: Use pre-allocated buffers for GEMM outputs
                self._v115_ensure_buffers(NS)
                hidden = self._buf_hidden[:, :NS, :]
                torch.matmul(x_flat.unsqueeze(0), self._all_fc1_w, out=hidden)
                hidden.add_(self._all_fc1_b.unsqueeze(1))
                F.relu(hidden, inplace=True)

                out_all = self._buf_out_all[:, :NS, :]
                torch.matmul(hidden, self._all_fc2_w, out=out_all)
                out_all.add_(self._all_fc2_b.unsqueeze(1))
            else:
                # V109 original: allocate every call
            ...
            if V115_GATHER_TOPK:
                # V115: gather-based top-2 weighted sum
                # Only collect top-2 expert outputs instead of multiplying all 8 by gate weights
                topk_idx_flat = topk_idx.reshape(NS, self.k)      # [NS, 2]
                topk_score_flat = topk_score.reshape(NS, self.k)   # [NS, 2]
                token_idx = torch.arange(NS, device=x.device).unsqueeze(1).expand(-1, self.k)  # [NS, 2]

                # Gather top-2 expert outputs: [NS, 2, 512]
                selected = out_all[topk_idx_flat, token_idx, :]
                # Weighted sum: [NS, 2, 512] * [NS, 2, 1] -> sum -> [NS, 512]
                out = (selected * topk_score_flat.unsqueeze(-1)).sum(dim=1)
                out = out.reshape(B, S, D)
            else:
                # V109 original: full 8-expert weighted sum
...
def main():
    ...
    # V115: Remove autocast — V14 already converts weights to FP16 explicitly.
    # autocast forces LayerNorm to FP32 (per PyTorch autocast policy), which
    # conflicts with V14's design intent of FP16 LayerNorm execution.
    # Removing autocast eliminates 16 unnecessary FP16→FP32→FP16 conversions
    # per forward pass (8 layers × 2 LayerNorms) and hundreds of autocast
    # dispatch queries.
    if V115_NO_AUTOCAST:
        autocast_ctx = nullcontext()
        _info("[INFO] V115: autocast disabled — using explicit FP16 path from V14")
    else:
        autocast_ctx = (
