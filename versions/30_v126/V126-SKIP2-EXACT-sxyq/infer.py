...
# V121: Skip 2 middle layers + bias-shift PCOC calibration
# V120 proved: skip 1 layer (4) + bias bake works: 42.10s, PCOC=1.0698, score_all=69.14
# V121: skip 2 layers (3,4) for more latency reduction
# PCOC correction: V119's exact method (sigmoid→scale→logit) to first measure the actual PCOC
# with 2 skipped layers, then we can bake the correct bias in V122
# For now, use a conservative correction factor based on extrapolation:
#   V117 skip 1 layer → PCOC=1.383, correction=0.7658 → PCOC_corrected=1.0698
#   V121 skip 2 layers → PCOC estimated ~1.6-1.8, correction estimated ~0.60-0.66
#   Start with correction=0.623 (target PCOC=1.07) and use V119's exact method
#   to get the precise measurement, then bake bias in next version
V121_SKIP_LAYERS = os.environ.get("GRAB_V121_SKIP_LAYERS", "3,4")
V121_PCOC_CORRECTION = float(os.environ.get("GRAB_V121_PCOC_CORRECTION", "0.623"))
...
class CTRModel(nn.Module):
    ...
    def forward(self, batch):
        ...
        # V121: PCOC calibration at MODEL OUTPUT level (same as V119, exact method)
        # Using V119's proven exact method: sigmoid→scale→logit
        # This adds ~5.4s latency but gives precise PCOC correction
        # Next version (V122) will bake the correct bias for zero-cost correction
        if not self.training and hasattr(self, '_v121_pcoc_correction') and self._v121_pcoc_correction != 1.0:
            probs = torch.sigmoid(pred_logits)
            scaled_probs = probs * self._v121_pcoc_correction
...
def load_model(ckpt_path=None, device='cuda:0'):
    ...
    # V121: Skip 2 middle layers
    skip_layers = []
    if V121_SKIP_LAYERS:
        try:
            skip_layers = [int(x.strip()) for x in V121_SKIP_LAYERS.split(',') if x.strip()]
        except ValueError:
            _warn(f"[WARNING] Invalid V121_SKIP_LAYERS: {V121_SKIP_LAYERS}")
            skip_layers = []
    if skip_layers:
        skip_layers = [i for i in skip_layers if 0 < i < num_layers - 1]
        model.seq_encoder.set_skip_layers(skip_layers)
        _info(f"[INFO] V121: skipping middle layers {skip_layers} (running {num_layers - len(skip_layers)}/{num_layers} layers)")

    # V121: Set PCOC correction factor for exact model-level correction
    if V121_PCOC_CORRECTION != 1.0:
        model._v121_pcoc_correction = V121_PCOC_CORRECTION
        _info(f"[INFO] V121: PCOC correction factor = {V121_PCOC_CORRECTION} (exact method, adds ~5s latency)")
