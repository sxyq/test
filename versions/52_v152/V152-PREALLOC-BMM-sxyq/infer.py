...
V109_DENSE_SMOE = os.environ.get("GRAB_V109_DENSE_SMOE", "0").lower() not in {"0", "false", "no"}
V147_INT8_SMOE = os.environ.get("GRAB_V147_INT8_SMOE", "0").lower() not in {"0", "false", "no"}
V150_GROUPED_BMM = os.environ.get("GRAB_V150_GROUPED_BMM", "0").lower() not in {"0", "false", "no"}
V151_NO_SYNC_BMM = os.environ.get("GRAB_V151_NO_SYNC_BMM", "0").lower() not in {"0", "false", "no"}
V152_PREALLOC_BMM = os.environ.get("GRAB_V152_PREALLOC_BMM", "1").lower() not in {"0", "false", "no"}
...
class SMoE(nn.Module):
    ...
    def _v109_prepare_dense_weights(self):
        ...
        # V152: Pre-allocate padded buffers for grouped BMM (eliminates per-batch allocation overhead)
        if V152_PREALLOC_BMM and not self.training:
            try:
                device = self._all_fc1_w.device
                dtype = self._all_fc1_w.dtype
                E = self.num_experts
                D_w = self._all_fc1_w.shape[1]
                self._v152_max_count = int(os.environ.get("GRAB_V152_MAX_COUNT", "2000"))
                self._v152_padded_x = torch.zeros(E, self._v152_max_count, D_w, device=device, dtype=dtype)
                self._v152_padded_scores = torch.zeros(E, self._v152_max_count, 1, device=device, dtype=dtype)
                _info(f"[INFO] V152: Pre-allocated buffers (max_count={self._v152_max_count})")
            except Exception as e:
                _warn(f"[WARNING] V152 pre-allocation failed: {e}")
                self._v152_max_count = 0
    ...
    def forward(self, x):
        ...
        if V152_PREALLOC_BMM and not self.training and self._v109_dense_weights_ready and self._v152_max_count > 0:
            # V152: Pre-allocated grouped BMM — eliminates per-batch memory allocation overhead
            # V151 failed because new_zeros allocation took ~8s; V152 uses pre-allocated zero_() buffers
            # No sync (estimated max_count), no allocation (pre-allocated), compute 2.67x less than V109
            x_flat = x.reshape(-1, D)
            NS = B * S
            E = self.num_experts
            k = self.k
            max_count = self._v152_max_count

            # Fall back to Dense if average count would exceed max_count (NS too large)
            if NS * k // E > max_count:
                # Dense path (V109)
                gate_weights = x_flat.new_zeros(NS, E)
                gate_weights.scatter_(1, topk_idx.reshape(NS, k), topk_score.reshape(NS, k))
                ...
                # Count and offsets (GPU only, no sync)
                counts = torch.bincount(expert_idx_sorted, minlength=E)
                offsets = torch.cumsum(counts, dim=0) - counts
                arange_nsk = torch.arange(NS * k, device=x.device)
                position_in_expert = arange_nsk - offsets[expert_idx_sorted]

                # Clamp positions (no sync, overflow probability ~0)
                position_clamped = torch.clamp(position_in_expert, 0, max_count - 1)

                # Reuse pre-allocated buffers (zero_() is much cheaper than new_zeros)
                self._v152_padded_x.zero_()
                self._v152_padded_scores.zero_()

                # Scatter tokens and scores into pre-allocated buffers
                self._v152_padded_x[expert_idx_sorted, position_clamped] = x_flat[token_idx_sorted]
                self._v152_padded_scores[expert_idx_sorted, position_clamped] = expert_score_sorted

                # Single bmm for all experts
                hidden = torch.bmm(self._v152_padded_x, self._all_fc1_w) + self._all_fc1_b.unsqueeze(1)
                hidden = F.relu(hidden)
                out_all = torch.bmm(hidden, self._all_fc2_w) + self._all_fc2_b.unsqueeze(1)
                out_all = out_all * self._v152_padded_scores

                # Gather and sum
                gathered_out = out_all[expert_idx_sorted, position_clamped]
                out_flat = x_flat.new_zeros(NS, D)
                out_flat.index_add_(0, token_idx_sorted, gathered_out)
                out = out_flat.reshape(B, S, D)

        elif V151_NO_SYNC_BMM and not self.training and self._v109_dense_weights_ready:
            # V151: No-sync grouped BMM — eliminates the GPU→CPU sync that killed V150
            # Key innovation: use estimated max_count = NS*k/E*2 (2x average, no sync needed)
            # Clamp positions to prevent out-of-bounds (overflow probability ~0 with 2x margin)
            # Compute: 2xNS (only top-2) vs V109's 8xNS = 4x less compute
            # No sync: 0 GPU→CPU syncs vs V150's 8 per batch
            x_flat = x.reshape(-1, D)
            NS = B * S
            E = self.num_experts
            k = self.k
            ...
            expert_idx_flat = topk_idx.reshape(-1)  # [NS*k]
            expert_score_flat = topk_score.reshape(-1, 1)  # [NS*k, 1]
            ...
            # Count tokens per expert and compute offsets (all on GPU, no sync)
            counts = torch.bincount(expert_idx_sorted, minlength=E)  # [E]
            offsets = torch.cumsum(counts, dim=0) - counts  # [E]

            # Position within each expert's group
            arange_nsk = torch.arange(NS * k, device=x.device)
            position_in_expert = arange_nsk - offsets[expert_idx_sorted]  # [NS*k]

            # V151: Use estimated max_count (NO SYNC!)
            # 1.5x the average count — overflow probability is ~0 (P(Z>18)≈0 for uniform gate)
            max_count = max(NS * k // E * 3 // 2, 100)

            # Clamp positions to prevent out-of-bounds (rare overflow tokens get incorrect output,
            # but probability is negligible with 2x margin)
            position_clamped = torch.clamp(position_in_expert, 0, max_count - 1)

            # Scatter tokens into padded buffer: [E, max_count, D]
            padded_x = x_flat.new_zeros(E, max_count, D)
            padded_x[expert_idx_sorted, position_clamped] = x_flat[token_idx_sorted]

            # Scatter scores into padded buffer: [E, max_count, 1]
            padded_scores = x_flat.new_zeros(E, max_count, 1)
            padded_scores[expert_idx_sorted, position_clamped] = expert_score_sorted

            # Single bmm for all experts: [E, max_count, D] @ [E, D, dim_ff] → [E, max_count, dim_ff]
            hidden = torch.bmm(padded_x, self._all_fc1_w) + self._all_fc1_b.unsqueeze(1)
            hidden = F.relu(hidden)
            # [E, max_count, dim_ff] @ [E, dim_ff, D] → [E, max_count, D]
            out_all = torch.bmm(hidden, self._all_fc2_w) + self._all_fc2_b.unsqueeze(1)

            # Weight by gate scores
            out_all = out_all * padded_scores  # [E, max_count, D]

            # Gather outputs using clamped positions
            gathered_out = out_all[expert_idx_sorted, position_clamped]  # [NS*k, D]
            out_flat = x_flat.new_zeros(NS, D)
            out_flat.index_add_(0, token_idx_sorted, gathered_out)
            out = out_flat.reshape(B, S, D)

        elif V150_GROUPED_BMM and not self.training and self._v109_dense_weights_ready:
            # V150: Grouped BMM SMoE — only compute top-k experts, single bmm for all experts
            # Key innovation: pad sorted tokens to [E, max_count, D], single bmm replaces 8 separate mm
            # Compute: 2xNS (only top-2) vs V109's 8xNS (all 8 experts) = 4x less compute
            # Kernels: ~9 (sort+scatter+bmm+relu+bmm+scatter) vs V109's ~5, but compute dominates
            x_flat = x.reshape(-1, D)
            NS = B * S
            E = self.num_experts
            k = self.k

            # Expand token-expert pairs: each token has k=2 experts
            token_idx_flat = (
                torch.arange(NS, device=x.device)
                .unsqueeze(1)
                .expand(-1, k)
            ...
            order = torch.argsort(expert_idx_flat)
            expert_idx_sorted = expert_idx_flat[order]  # [NS*k]
            token_idx_sorted = token_idx_flat[order]  # [NS*k]
            expert_score_sorted = expert_score_flat[order]  # [NS*k, 1]

            # Count tokens per expert and compute offsets
            counts = torch.bincount(expert_idx_sorted, minlength=E)  # [E]
            offsets = torch.cumsum(counts, dim=0) - counts  # [E]
            ...
            # One GPU→CPU sync to get max_count (1 per layer vs V132's 16)
            ...
            if max_count == 0:
                ...
                # Scatter tokens into padded buffer: [E, max_count, D]
                padded_x = x_flat.new_zeros(E, max_count, D)
                padded_x[expert_idx_sorted, position_in_expert] = x_flat[token_idx_sorted]

                # Scatter scores into padded buffer: [E, max_count, 1]
                padded_scores = x_flat.new_zeros(E, max_count, 1)
                padded_scores[expert_idx_sorted, position_in_expert] = expert_score_sorted
                ...
                # Scatter back: gather outputs and sum over k experts per token
                gathered_out = out_all[expert_idx_sorted, position_in_expert]  # [NS*k, D]
                out_flat = x_flat.new_zeros(NS, D)
                out_flat.index_add_(0, token_idx_sorted, gathered_out)
                out = out_flat.reshape(B, S, D)
...
def load_model(ckpt_path=None, device='cuda:0'):
    ...
    # V109/V150/V151/V152: Pre-stack expert weights for batched/grouped GEMM (must be after .to(dev) and FP16 conversion)
    if V109_DENSE_SMOE or V150_GROUPED_BMM or V151_NO_SYNC_BMM or V152_PREALLOC_BMM:
        for moe in model.seq_encoder.moe:
            moe._v109_prepare_dense_weights()
        _info("[INFO] Expert weights pre-stacked for batched/grouped GEMM")
