...
# V126: Skip 2 middle layers + exact PCOC calibration (V126 proven config, no compile)
# V126 proved: skip 2 layers (3,4) + exact correction=0.623 → PCOC=0.99, score=69.34 ✓
# V126 is a clean re-submission of V126's proven configuration
V126_SKIP_LAYERS = os.environ.get("GRAB_V126_SKIP_LAYERS", "3,4")
V126_PCOC_CORRECTION = float(os.environ.get("GRAB_V126_PCOC_CORRECTION", "0.623"))
...
class CTRModel(nn.Module):
    ...
    def forward(self, batch):
        ...
        # V126: PCOC calibration at MODEL OUTPUT level (same as V119, exact method)
        ...
        if not self.training and hasattr(self, '_v126_pcoc_correction') and self._v126_pcoc_correction != 1.0:
            probs = torch.sigmoid(pred_logits)
            scaled_probs = probs * self._v126_pcoc_correction
...
def load_model(ckpt_path=None, device='cuda:0'):
    ...
    # V126: Skip 2 middle layers
    skip_layers = []
    if V126_SKIP_LAYERS:
        try:
            skip_layers = [int(x.strip()) for x in V126_SKIP_LAYERS.split(',') if x.strip()]
        except ValueError:
            _warn(f"[WARNING] Invalid V126_SKIP_LAYERS: {V126_SKIP_LAYERS}")
            skip_layers = []
    if skip_layers:
        skip_layers = [i for i in skip_layers if 0 < i < num_layers - 1]
        model.seq_encoder.set_skip_layers(skip_layers)
        _info(f"[INFO] V126: skipping middle layers {skip_layers} (running {num_layers - len(skip_layers)}/{num_layers} layers)")

    # V126: Set PCOC correction factor for exact model-level correction
    if V126_PCOC_CORRECTION != 1.0:
        model._v126_pcoc_correction = V126_PCOC_CORRECTION
        _info(f"[INFO] V126: PCOC correction factor = {V126_PCOC_CORRECTION} (exact method, adds ~5s latency)")
