...
# V125: Skip 3 middle layers + exact PCOC calibration
# V123/V124 both zero — V123 likely AUC<0.65 from skipping 3 layers, V124 torch.compile crash
# V125: skip 3 layers with exact correction to test if AUC stays above 0.65
# correction=0.5724 is estimated; exact method guarantees PCOC correction regardless
V125_SKIP_LAYERS = os.environ.get("GRAB_V125_SKIP_LAYERS", "3,4,5")
V125_PCOC_CORRECTION = float(os.environ.get("GRAB_V125_PCOC_CORRECTION", "0.5724"))
...
class CTRModel(nn.Module):
    ...
    def forward(self, batch):
        ...
        # V125: PCOC calibration at MODEL OUTPUT level (same as V119, exact method)
        ...
        if not self.training and hasattr(self, '_v125_pcoc_correction') and self._v125_pcoc_correction != 1.0:
            probs = torch.sigmoid(pred_logits)
            scaled_probs = probs * self._v125_pcoc_correction
...
def load_model(ckpt_path=None, device='cuda:0'):
    ...
    # V125: Skip 2 middle layers
    skip_layers = []
    if V125_SKIP_LAYERS:
        try:
            skip_layers = [int(x.strip()) for x in V125_SKIP_LAYERS.split(',') if x.strip()]
        except ValueError:
            _warn(f"[WARNING] Invalid V125_SKIP_LAYERS: {V125_SKIP_LAYERS}")
            skip_layers = []
    if skip_layers:
        skip_layers = [i for i in skip_layers if 0 < i < num_layers - 1]
        model.seq_encoder.set_skip_layers(skip_layers)
        _info(f"[INFO] V125: skipping middle layers {skip_layers} (running {num_layers - len(skip_layers)}/{num_layers} layers)")

    # V125: Set PCOC correction factor for exact model-level correction
    if V125_PCOC_CORRECTION != 1.0:
        model._v125_pcoc_correction = V125_PCOC_CORRECTION
        _info(f"[INFO] V125: PCOC correction factor = {V125_PCOC_CORRECTION} (exact method, adds ~5s latency)")
