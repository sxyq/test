...
V114_BLOCK_DIAG_ATTN = os.environ.get("GRAB_V114_BLOCK_DIAG_ATTN", "1").lower() not in {"0", "false", "no"}
V114_REP_EMB_VIEW = os.environ.get("GRAB_V114_REP_EMB_VIEW", "1").lower() not in {"0", "false", "no"}
...
class RepEncoder(nn.Module):
    ...
    def forward_sparse_embedding_bag(self, batch):
        ...
        if V114_REP_EMB_VIEW:
            # V114: embedding_bag returns [slot_num * num_tokens, emb_dim] in slot-major order
            # (slot0_all_tokens, slot1_all_tokens, ...). We need [num_tokens, slot_num * emb_dim].
            # Original: view + transpose + reshape causes implicit contiguous() copy.
            # Optimized: permute + contiguous + view — same logic but explicit about the copy.
            fused_embs = fused_embs.view(self.slot_num, -1, self.emb_dim)
            fused_embs = fused_embs.permute(1, 0, 2).contiguous()
            fused_embs = fused_embs.view(-1, self.slot_num * self.emb_dim)
        else:
...
def per_user_causal_sdpa(q, k, v, user_offsets_cpu):
    ...
    # Path 2: V114 block-diagonal causal mask — single SDPA call, no Python loop
    if V114_BLOCK_DIAG_ATTN:
        total_len = q.shape[2]
        device = q.device

        # Build user_id for each position using searchsorted
        positions = torch.arange(total_len, device=device)
        user_ids = torch.searchsorted(user_offsets_cpu[1:].to(device), positions, right=True)

        # Block-diagonal causal mask:
        # mask[i,j] = True if same_user AND j <= i
        same_user = user_ids.unsqueeze(0) == user_ids.unsqueeze(1)  # [T, T]
        causal = positions.unsqueeze(0) <= positions.unsqueeze(1)    # [T, T] — j <= i means col <= row
        # Wait: positions[j] <= positions[i] means j <= i (since positions = arange)
        # Actually: we want mask[i,j] = True if user_ids[i]==user_ids[j] AND j <= i
        # positions[j] <= positions[i] is just j <= i, which is correct for causal
        # But we need: for row i, column j is allowed if same user and j <= i
        # So: allowed = same_user & (j <= i) = same_user & causal
        # SDPA attn_mask: True = allowed, False = masked out
        attn_mask = same_user & causal  # [T, T] bool

        # Reshape for SDPA: [1, 1, T, T] (broadcast over batch and heads)
    ...
    # Path 3: per-user SDPA with is_causal=True (original fallback)
    outputs = []
