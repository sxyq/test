...
# V131: Skip 5 middle layers + bias-shift PCOC calibration (ZERO extra latency)
# V130 proved: skip 4 layers + bias bake → PCOC=1.094, AUC=0.749, latency=37.75s
# V131 extrapolates: skip 5 layers → correction≈0.33, bias=log(0.33)≈-1.109
# PCOC trend: skip1=0.7658, skip2=0.6664, skip3=0.5269, skip4=0.417 → skip5≈0.33
# Risk: AUC may drop below 0.65, but worth trying for ~35s latency
V131_SKIP_LAYERS = os.environ.get("GRAB_V131_SKIP_LAYERS", "1,2,3,4,5")
V131_LOGIT_BIAS = float(os.environ.get("GRAB_V131_LOGIT_BIAS", str(math.log(0.33))))
