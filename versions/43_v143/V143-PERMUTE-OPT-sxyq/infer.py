...
def per_user_causal_sdpa_flash_direct(q, k, v, user_offsets_cpu):
    """V143: Flash attention path with zero permute overhead.

    Args:
        q, k, v: [total_tokens, n_heads, head_dim] — already in flash_attn format
        user_offsets_cpu: [num_users+1] CPU LongTensor

    Returns:
        attn_out: [total_tokens, n_heads, head_dim]
    """
    orig_dtype = q.dtype
    q_fa = q.half().contiguous()
    k_fa = k.half().contiguous()
    v_fa = v.half().contiguous()
    max_seqlen = int((user_offsets_cpu[1:] - user_offsets_cpu[:-1]).max())
    cu_seqlens = user_offsets_cpu.to(torch.int32)
    out_fa = _flash_attn_varlen_func(
        q_fa, k_fa, v_fa,
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x, extension=None, user_offsets_cpu=None):
        ...
        for i in range(self.num_layers):
            ...
            # V143: When flash_attn is available, skip all permutes
            # Old path: view→permute→split→(flash_attn: squeeze+permute)→(return: permute+unsqueeze)→permute+reshape = 6 permutes
            # New path: view→split→flash_attn→view = 0 permutes
            if _FLASH_ATTN_AVAILABLE and V103_PER_USER_CAUSAL_SDPA and user_offsets_cpu is not None:
                # Directly produce [S, H, 3*hd] format for flash_attn
                qkv = qkv.view(S, self.n_heads, 3 * self.head_dim)
                q, k, v = torch.split(qkv, self.head_dim, dim=-1)
                attn_out = per_user_causal_sdpa_flash_direct(q, k, v, user_offsets_cpu)
                attn_out = attn_out.view(1, S, D)
            else:
