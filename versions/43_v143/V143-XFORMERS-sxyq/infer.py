...
# V143: xformers is the preferred path for V100 (SM70) — native IO-aware attention
try:
    from xformers.ops import fmha as _xformers_fmha
...
def per_user_causal_sdpa(q, k, v, user_offsets_cpu):
    ...
    # V143 Path 0: xformers memory_efficient_attention (SM70 native, preferred for V100)
    if _XFORMERS_AVAILABLE:
        # xformers expects [seq, heads, dim] (no batch dim)
        q_xf = q.squeeze(0).permute(1, 0, 2).half().contiguous()
        k_xf = k.squeeze(0).permute(1, 0, 2).half().contiguous()
        v_xf = v.squeeze(0).permute(1, 0, 2).half().contiguous()
        # Build block-diagonal causal mask from user offsets
        seqlens = [offsets[i+1] - offsets[i] for i in range(num_users) if offsets[i+1] > offsets[i]]
        attn_bias = _xformers_fmha.BlockDiagonalMask.from_seqlens(seqlens).make_causal()
        out_xf = _xformers_fmha.memory_efficient_attention(
            q_xf, k_xf, v_xf, attn_bias=attn_bias
        )
        # [S, H, D] -> [1, H, S, D]
        return out_xf.permute(1, 0, 2).unsqueeze(0).to(q.dtype)
