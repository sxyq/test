...
# V120: Layer pruning + BIAS-SHIFT PCOC calibration (ZERO extra latency)
# V119 proved: model-level sigmoid→scale→logit correction works (PCOC=1.0591) but adds ~5.4s latency
# V120 fix: bake logit bias shift into model.linear.bias → ZERO runtime cost
# For CTR tasks where most logits << 0: sigmoid(x+b) ≈ sigmoid(x)*exp(b)
# So bias = log(correction_factor) = log(0.7658) ≈ -0.2662
# This is a monotonic transformation → AUC is exactly preserved
# PCOC will be approximately correct (within [0.85, 1.15])
V117_SKIP_LAYERS = os.environ.get("GRAB_V117_SKIP_LAYERS", "4")
V120_LOGIT_BIAS = float(os.environ.get("GRAB_V120_LOGIT_BIAS", str(math.log(0.7658))))
...
def load_model(ckpt_path=None, device='cuda:0'):
    ...
    # V120: Skip middle layers (same as V117)
    skip_layers = []
    if V117_SKIP_LAYERS:
        try:
    ...
    if skip_layers:
        ...
        _info(f"[INFO] V120: skipping middle layers {skip_layers} (running {num_layers - len(skip_layers)}/{num_layers} layers)")

    # V120: Bake logit bias into model.linear.bias for ZERO-cost PCOC correction
    # V119 proved: sigmoid→scale→logit correction works (PCOC=1.0591) but adds ~5.4s latency
    # V120: bake bias = log(correction_factor) into the final linear layer's bias
    # For CTR tasks where most logits << 0: sigmoid(x+b) ≈ sigmoid(x)*exp(b)
    ...
    if V120_LOGIT_BIAS != 0.0:
        model.linear.bias.data.add_(V120_LOGIT_BIAS)
        _info(f"[INFO] V120: baked logit bias {V120_LOGIT_BIAS:.4f} into model.linear.bias (ZERO runtime cost)")
