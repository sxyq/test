...
V150_GROUPED_BMM = os.environ.get("GRAB_V150_GROUPED_BMM", "1").lower() not in {"0", "false", "no"}
...
class SMoE(nn.Module):
    ...
    def forward(self, x):
        ...
        if V150_GROUPED_BMM and not self.training and self._v109_dense_weights_ready:
...
def load_model(ckpt_path=None, device='cuda:0'):
    ...
    # V109/V150: Pre-stack expert weights for batched/grouped GEMM (must be after .to(dev) and FP16 conversion)
    if V109_DENSE_SMOE or V150_GROUPED_BMM:
