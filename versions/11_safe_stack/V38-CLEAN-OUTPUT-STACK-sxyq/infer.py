    ...
    def forward(self, batch):
        if self.v24_sparse_embedding_active:
            try:
                ...
                print(f"[WARNING] V24 sparse embedding path failed once; fallback to V16 segment_reduce path: {type(exc).__name__}: {exc}")
...
def apply_v14_fp16_weights(model):
    ...
    if not V14_FP16_WEIGHTS_ENABLED:
        print("[INFO] V14 FP16 weight path disabled by GRAB_V14_FP16_WEIGHTS")
        return converted
    ...
    print(f"[INFO] V14 FP16 weight path enabled: converted {converted} seq_encoder Linear/LayerNorm modules")
    return converted
...
def apply_v15_fp16_deep(model):
    ...
    if not V15_FP16_DEEP_ENABLED:
        print("[INFO] V15 FP16 deep path disabled by GRAB_V15_FP16_DEEP")
        return 0
    model.rep_encoder.linear.half()
    model.rep_encoder.v15_fp16_linear_active = True
    print("[INFO] V15 FP16 deep path enabled: converted RepEncoder.linear")
    return 1
...
def apply_v16_fp16_embedding(model):
    ...
    if not V16_FP16_EMB_ENABLED:
        print("[INFO] V16 FP16 embedding path disabled by GRAB_V16_FP16_EMB")
        return 0
    model.rep_encoder.emb.half()
    model.rep_encoder.v16_fp16_embedding_active = True
    print("[INFO] V16 FP16 embedding path enabled: converted RepEncoder.emb")
    return 1
...
def apply_v24_sparse_embedding(model):
    ...
    if not V24_SPARSE_EMB_ENABLED:
        print("[INFO] V24 sparse embedding path disabled by GRAB_V24_SPARSE_EMB")
        return 0
    if not hasattr(F, "embedding_bag"):
        print("[WARNING] V24 sparse embedding path unavailable: torch.nn.functional.embedding_bag missing")
        return 0
    model.rep_encoder.v24_sparse_embedding_active = True
    print("[INFO] V24 sparse embedding path enabled: fused slot aggregation with embedding_bag")
    return 1
