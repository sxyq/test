...
def load_model(ckpt_path=None, device='cuda:0'):
    ...
    # V142: Compile each SMoE module individually (stable shape, pure tensor ops)
    # Full-model compile failed in V124, but SMoE is compile-friendly.
    try:
        for moe in model.seq_encoder.moe:
            moe.forward = torch.compile(
                moe.forward,
                mode="max-autotune",
                dynamic=False,
                fullgraph=False,
            )
        _info("[INFO] V142 torch.compile: compiled SMoE forward (max-autotune, dynamic=False)")
    except Exception as exc:
        _warn(f"[WARNING] V142 torch.compile SMoE failed: {type(exc).__name__}: {exc}")
