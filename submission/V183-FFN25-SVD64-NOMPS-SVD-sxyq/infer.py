...
# V183: keep V182's CPU-oriented structure, but skip low-rank SVD caches on MPS.
# This is a local feasibility branch for Apple Silicon validation, aimed at
# avoiding MPS -> CPU fallback in torch.linalg.svd during prep.
...
class SMoE(nn.Module):
    ...
    def _v109_prepare_dense_weights(self):
        ...
        if V168_SVD_LOWRANK and not self.training:
            try:
                if self._all_fc1_w.device.type == "mps":
                    raise RuntimeError("V183 local branch: skip SVD cache build on MPS to avoid CPU fallback")
