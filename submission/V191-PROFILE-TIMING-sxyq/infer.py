...
# V191: profile-only candidate. It keeps V185 prediction semantics and emits
# coarse end-to-end timing buckets for local/cloud diagnosis.
V191_PROFILE_TIMING = os.environ.get("GRAB_V191_PROFILE_TIMING", "1").lower() not in {"0", "false", "no"}
...
def main():
    ...
    time_sum = 0.0
    profile_prepare_sum = 0.0
    profile_forward_sum = 0.0
    profile_collect_sum = 0.0
    profile_batches = 0
    ...
    def run_one_batch(batch):
        ...
        nonlocal profile_prepare_sum, profile_forward_sum, profile_collect_sum
        nonlocal profile_batches, profile_warmup_batches
        batch_prepare_start = time.time() if V191_PROFILE_TIMING else None
        ...
        if V191_PROFILE_TIMING:
            profile_prepare_sum += time.time() - batch_prepare_start
        is_warmup = not warmup_done
        t_start = None if is_warmup else time.time()
        forward_start = time.time() if V191_PROFILE_TIMING else None
        try:
        ...
        if V191_PROFILE_TIMING:
            ...
            profile_forward_sum += time.time() - forward_start
        ...
        collect_start = time.time() if V191_PROFILE_TIMING else None
        ...
        if V191_PROFILE_TIMING:
            profile_collect_sum += time.time() - collect_start
            profile_batches += 1
    ...
    if V191_PROFILE_TIMING:
        _info(f'[PROFILE] data_prepare_wall: {data_ready_ts - load_start_ts:.4f}s', always=True)
        _info(f'[PROFILE] model_load_wall: {model_ready_ts - model_load_start:.4f}s', always=True)
        _info(f'[PROFILE] batch_prepare_sum: {profile_prepare_sum:.4f}s', always=True)
        _info(f'[PROFILE] forward_sum_sync: {profile_forward_sum:.4f}s', always=True)
        _info(f'[PROFILE] collect_sum: {profile_collect_sum:.4f}s', always=True)
        _info(f'[PROFILE] batches: {profile_batches}, warmup_batches: {profile_warmup_batches}', always=True)
