...
# V156: xformers memory_efficient_attention - SM70 (V100) native support
# xformers is the most reliable attention acceleration backend for V100,
# unlike flash_attn which may silently fall back to SDPA on SM70.
try:
    from xformers.ops import memory_efficient_attention as _xformers_me_attention
...
V153_PRUNE_FFN = os.environ.get("GRAB_V153_PRUNE_FFN", "0").lower() not in {"0", "false", "no"}
V153_PRUNE_RATIO = float(os.environ.get("GRAB_V153_PRUNE_RATIO", "0.25"))
# V156: head_dim pruning disabled by default (potential compliance violation)
# head_dim is a derived architecture parameter (d_model // n_heads); modifying it
# violates "不可修改模型组网和相关参数". LASP (ICLR 2026) also advises against
# pruning Q/K/V sub-dimensions. Set to "1" only for experiments.
V155_PRUNE_HEAD_DIM = os.environ.get("GRAB_V155_PRUNE_HEAD_DIM", "0").lower() not in {"0", "false", "no"}
V155_HEAD_DIM_RATIO = float(os.environ.get("GRAB_V155_HEAD_DIM_RATIO", "0.25"))
V153_LOGIT_BIAS = float(os.environ.get("GRAB_V153_LOGIT_BIAS", "0.0"))
# V156: xformers memory_efficient_attention for V100 (SM70) native support
# Priority: xformers > flash_attn > per-user SDPA loop
# V157A: xformers disabled by default (BlockDiagonalMask may cause PCOC=0 on V100)
V156_XFORMERS_ATTENTION = os.environ.get("GRAB_V156_XFORMERS_ATTENTION", "0").lower() not in {"0", "false", "no"}
# V157: Sparse SMoE - only compute activated experts (Top-2) instead of all 8
# Eliminates 4x over-computation of V109 Dense SMoE
# Priority: V157 sparse > V109 dense > original loop
V157_SPARSE_SMOE = os.environ.get("GRAB_V157_SPARSE_SMOE", "1").lower() not in {"0", "false", "no"}
...
def configure_runtime():
    ...
    try:
        ...
        pass
    # V147: Allow FP16 reduced precision reduction for faster GEMM on V100
    try:
...
def per_user_causal_sdpa(q, k, v, user_offsets_cpu):
    ...
    V156: Priority order: xformers (SM70 native) > flash_attn > per-user SDPA loop
    """
    num_users = user_offsets_cpu.size(0) - 1
    offsets = user_offsets_cpu.tolist()

    # Path 0: xformers memory_efficient_attention (V100/SM70 native support)
    # Unlike flash_attn which may silently fall back on SM70, xformers explicitly
    # supports SM70+ and is the most reliable attention backend for V100.
    if V156_XFORMERS_ATTENTION and _XFORMERS_AVAILABLE:
        # xformers expects [batch, seq_len, num_heads, head_dim]
        # current: [1, n_heads, total_tokens, head_dim] -> permute to [1, total_tokens, n_heads, head_dim]
        q_xf = q.permute(0, 2, 1, 3).half().contiguous()
        k_xf = k.permute(0, 2, 1, 3).half().contiguous()
        v_xf = v.permute(0, 2, 1, 3).half().contiguous()
        # Build BlockDiagonalMask from per-user sequence lengths
        seq_lens = (user_offsets_cpu[1:] - user_offsets_cpu[:-1]).tolist()
        # Filter out zero-length users for mask construction
        non_zero_lens = [s for s in seq_lens if s > 0]
        attn_bias = BlockDiagonalMask.from_seqlens(non_zero_lens).make_causal()
        out_xf = _xformers_me_attention(q_xf, k_xf, v_xf, attn_bias=attn_bias)
        # [1, total_tokens, n_heads, head_dim] -> [1, n_heads, total_tokens, head_dim]
        return out_xf.permute(0, 2, 1, 3).to(q.dtype)
...
class SMoE(nn.Module):
    ...
    def _v109_prepare_dense_weights(self):
        ...
        # V153: Structured pruning of SMoE expert hidden dimension
        # Reduces dim_ff by prune_ratio (e.g., 1024→768 for 25% pruning)
        # Zero overhead: just smaller weight matrices, no extra kernels
        # Importance metric: ||fc1[row]|| * ||fc2[:, col]|| (product of connected weights)
        if V153_PRUNE_FFN and not self.training:
            try:
                E = self.num_experts
                dim_ff = self._all_fc1_w.shape[2]  # current dim_ff
                keep_count = int(dim_ff * (1.0 - V153_PRUNE_RATIO))
                if keep_count < dim_ff:
                    with torch.no_grad():
                        # Compute importance per hidden neuron: [E, dim_ff]
                        fc1_norms = self._all_fc1_w.norm(dim=1)  # [E, dim_ff] (norm over D)
                        fc2_norms = self._all_fc2_w.norm(dim=2)  # [E, dim_ff] (norm over D)
                        importance = fc1_norms * fc2_norms  # [E, dim_ff]

                        # Select top-k neurons per expert
                        keep_indices = importance.topk(keep_count, dim=1).indices.sort(dim=1).values  # [E, keep_count]

                        # Prune fc1 weights and biases: [E, D, dim_ff] → [E, D, keep_count]
                        # gather along dim=2 using keep_indices
                        ...
                        # Prune fc2 weights: [E, dim_ff, D] → [E, keep_count, D]
                        # gather along dim=1 using keep_indices
                        ...
                        # fc2 bias is [E, D], not affected by dim_ff pruning

                    _info(f"[INFO] V153: Pruned SMoE dim_ff {dim_ff}→{keep_count} ({V153_PRUNE_RATIO*100:.0f}% pruning, zero overhead)")
                else:
    ...
    def forward(self, x):
        ...
        if V157_SPARSE_SMOE and not self.training and self._v109_dense_weights_ready:
            # V157: Sparse SMoE - only compute activated experts (Top-2)
            # Eliminates 4x over-computation of V109 Dense SMoE
            # Uses pre-stacked dense weights but skips non-activated experts
            x_flat = x.reshape(-1, D)  # [NS, D]
            NS = B * S

            # Flatten topk: [NS, k] -> [NS*k]
            topk_idx_flat = topk_idx.reshape(-1)  # [NS*k]
            topk_score_flat = topk_score.reshape(-1)  # [NS*k]
            token_ids = torch.arange(NS, device=x.device).repeat_interleave(self.k)  # [NS*k]
            ...
            # Count tokens per expert (one CPU-GPU sync, then iterate without sync)
            counts = torch.bincount(expert_idx_sorted, minlength=self.num_experts)
            starts = torch.cumsum(counts, dim=0) - counts
            counts_list = counts.tolist()
            starts_list = starts.tolist()

            # Get weights (use INT8 dequant if active)
            ...
            for i in range(self.num_experts):
                ...
                cur_score = score_sorted[start:start + count].unsqueeze(-1).to(x_flat.dtype)  # [count, 1] match dtype
                selected_x = x_flat.index_select(0, cur_token_idx)  # [count, D]
                # Expert FFN: fc1 -> relu -> fc2 (using pre-stacked weights)
        ...
        elif V109_DENSE_SMOE and not self.training and self._v109_dense_weights_ready:
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x, extension=None, user_offsets_cpu=None):
        ...
        for i in range(self.num_layers):
            ...
            attn_out = attn_out.permute(0, 2, 1, 3).reshape(B, S, self.n_heads * self.head_dim)
...
def load_model(ckpt_path=None, device='cuda:0'):
    ...
    # V155: Prune attention head_dim (64→48, 25% reduction in QKV+OutProj+attention compute)
    # Zero overhead: just smaller weight matrices, same as V153 SMoE pruning
    if V155_PRUNE_HEAD_DIM:
        try:
            seq_enc = model.seq_encoder
            n_heads = seq_enc.n_heads
            old_head_dim = seq_enc.head_dim
            d_model = seq_enc.d_model
            new_head_dim = int(old_head_dim * (1.0 - V155_HEAD_DIM_RATIO))
            if new_head_dim < old_head_dim and new_head_dim > 0:
                with torch.no_grad():
                    for i in range(seq_enc.num_layers):
                        ...
                        # Importance per head_dim index: [old_head_dim]
                        qkv_imp = qkv_w.norm(dim=(0, 1, 3))  # [old_head_dim]
                        out_imp = out_w.norm(dim=(0, 1))  # [old_head_dim]
                        importance = qkv_imp * out_imp
                        keep = importance.topk(new_head_dim).indices.sort().values  # [new_head_dim]
                        # Prune QKV weight and bias
                        ...
                        # Prune OutProj weight (bias unchanged, it's [d_model])
                        out_w_new = out_w[:, :, keep].contiguous().view(d_model, -1)
                        outp.weight.data = out_w_new
                    # Update head_dim
                    seq_enc.head_dim = new_head_dim
                _info(f"[INFO] V155: Pruned head_dim {old_head_dim}→{new_head_dim} (QKV+OutProj+attn 25% less compute, zero overhead)")
            else:
                _info("[INFO] V155: head_dim pruning ratio too small, skipping")
        except Exception as e:
            _warn(f"[WARNING] V155 head_dim pruning failed: {e}, using full head_dim")
    ...
    # V153: Bake logit bias for PCOC correction (zero runtime cost)
    # Pruning may shift PCOC; bias baking corrects it with a single add to the final linear bias
    if V153_LOGIT_BIAS != 0.0:
        model.linear.bias.data.add_(V153_LOGIT_BIAS)
        _info(f"[INFO] V153: baked logit bias {V153_LOGIT_BIAS:.4f} into model.linear.bias (ZERO runtime cost)")
