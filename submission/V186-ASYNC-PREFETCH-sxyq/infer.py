...
# V186: overlap CPU->GPU transfer with compute using a dedicated CUDA prefetch
# stream. This is an I/O/runtime optimization only; it does not change batch
# contents, batch order, or prediction semantics.
V186_ASYNC_PREFETCH = os.environ.get("GRAB_V186_ASYNC_PREFETCH", "1").lower() not in {"0", "false", "no"}
...
def iter_inference_batches(iterable, dev, use_cuda):
    if not (use_cuda and V186_ASYNC_PREFETCH and torch.cuda.is_available()):
