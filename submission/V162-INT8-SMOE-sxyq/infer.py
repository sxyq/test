...
V162_INT8_SMOE = os.environ.get("GRAB_V162_INT8_SMOE", "1").lower() not in {"0", "false", "no"}
...
class SMoE(nn.Module):
    ...
    def _v109_prepare_dense_weights(self):
        ...
        # V162: INT8 quantize expert weights — 2x smaller → better L2 cache utilization
        if V162_INT8_SMOE and not self.training:
            try:
                with torch.no_grad():
                    fc1_absmax = self._all_fc1_w.abs().amax(dim=(1, 2), keepdim=True)
                    fc2_absmax = self._all_fc2_w.abs().amax(dim=(1, 2), keepdim=True)
                    self._v162_fc1_scale = (fc1_absmax / 127.0).clamp(min=1e-8)
                    self._v162_fc2_scale = (fc2_absmax / 127.0).clamp(min=1e-8)
                    self._v162_fc1_int8 = torch.round(self._all_fc1_w / self._v162_fc1_scale).to(torch.int8)
                    self._v162_fc2_int8 = torch.round(self._all_fc2_w / self._v162_fc2_scale).to(torch.int8)
                    # Delete FP16 weights to free memory
                    del self._all_fc1_w, self._all_fc2_w
                _info(f"[INFO] V162: INT8 quantized expert weights (fc1 {self._v162_fc1_int8.shape}, fc2 {self._v162_fc2_int8.shape})")
            except Exception as e:
                _warn(f"[WARNING] V162 INT8 quantization failed: {e}, using FP16 weights")
                self._v162_int8_active = False
    ...
    def forward(self, x):
        ...
        if V109_DENSE_SMOE and not self.training and self._v109_dense_weights_ready:
            ...
            gate_weights.scatter_(1, topk_idx.reshape(NS, self.k), topk_score.reshape(NS, self.k).to(gate_weights.dtype))

            # V162: Dequantize INT8 weights to FP16 on-the-fly
            if V162_INT8_SMOE and hasattr(self, '_v162_fc1_int8'):
                fc1_w = self._v162_fc1_int8.to(x_flat.dtype) * self._v162_fc1_scale.to(x_flat.dtype)
                fc2_w = self._v162_fc2_int8.to(x_flat.dtype) * self._v162_fc2_scale.to(x_flat.dtype)
            else:
