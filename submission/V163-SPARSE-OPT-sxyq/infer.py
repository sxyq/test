...
class SMoE(nn.Module):
    ...
    def forward(self, x):
        ...
        if V109_DENSE_SMOE and not self.training and self._v109_dense_weights_ready:
            ...
            # V163: Optimized sparse path — tolist() prefetch + pre-stacked weights
            ...
            counts_list = counts.tolist()  # one CPU-GPU sync, then iterate without sync
            starts_list = starts.tolist()

            # Use pre-stacked weights if available (faster than self.experts[i])
            use_stacked = self._v109_dense_weights_ready
            if use_stacked:
            ...
            for i in range(self.num_experts):
                ...
                if use_stacked:
                    hidden = torch.matmul(selected_x, fc1_w[i]) + self._all_fc1_b[i]
                    hidden = F.relu(hidden)
                    expert_out = torch.matmul(hidden, fc2_w[i]) + self._all_fc2_b[i]
                else:
