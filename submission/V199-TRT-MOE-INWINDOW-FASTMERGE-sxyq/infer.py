...
BAIDU_LOGIT_BIAS = float(os.environ.get("BAIDU_LOGIT_BIAS", "0.0"))
...
def merge_user_batches(batches):
    ...
    user_offset_parts = [torch.zeros(1, dtype=batches[0]["user_offsets"].dtype)]
    token_base = 0
    for batch in batches:
        offsets = batch["user_offsets"]
        user_offset_parts.append(offsets[1:] + token_base)
        token_base += int(offsets[-1])
    merged["user_offsets"] = torch.cat(user_offset_parts, dim=0)

    for slot in range(1, 29):
        values_list = []
        offset_parts = [torch.zeros(1, dtype=batches[0][slot][1].dtype)]
        value_base = 0
        for batch in batches:
            values, slot_offsets = batch[slot]
            values_list.append(values)
            offset_parts.append(slot_offsets[1:] + value_base)
            value_base += int(slot_offsets[-1])
        merged[slot] = (
            torch.cat(values_list, dim=0),
            torch.cat(offset_parts, dim=0),
        )
...
def main():
    ...
    # Rule-aligned batching: batch grouping is allowed, but it must happen
    # inside the timed inference region rather than during the preload stage.
    ...
    if group_token_cap > 0:
        ...
        print(f'[INFO] grouped batches in-window: {original_batch_count} -> {len(all_batches)} '
    ...
    time_sum += time.time() - group_start
