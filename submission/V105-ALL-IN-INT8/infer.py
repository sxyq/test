...
V33_CAUSAL_TRI_CACHE_ENABLED = os.environ.get("GRAB_V33_CAUSAL_TRI_CACHE", "0").lower() not in {"0", "false", "no"}
V33_CAUSAL_TRI_CACHE_MAX = int(os.environ.get("GRAB_V33_CAUSAL_TRI_CACHE_MAX", "128"))
V33_CAUSAL_TRI_CACHE_MAX_BYTES = int(os.environ.get("GRAB_V33_CAUSAL_TRI_CACHE_MAX_BYTES", str(128 * 1024 * 1024)))
...
V104_BATCHED_SMOE_ENABLED = os.environ.get("GRAB_V104_BATCHED_SMOE", "1").lower() not in {"0", "false", "no"}
V105_BATCH_SIZE_1 = os.environ.get("GRAB_V105_BATCH_SIZE_1", "1").lower() not in {"0", "false", "no"}
V105_FP16_FINAL_LINEAR = os.environ.get("GRAB_V105_FP16_FINAL_LINEAR", "1").lower() not in {"0", "false", "no"}
V105_INT8_WEIGHT = os.environ.get("GRAB_V105_INT8_WEIGHT", "1").lower() not in {"0", "false", "no"}
...
def scaled_dot_product(q, k, v, extension):
    is_causal = bool(extension is not None and extension.get("is_causal", False))
    user_offsets = extension.get("user_offsets") if extension is not None else None

    # Per-user causal SDPA: split by user_offsets, each user gets is_causal=True
    if is_causal and user_offsets is not None:
        offsets_cpu = user_offsets.cpu().tolist()
        num_users = len(offsets_cpu) - 1

        # Path 1: flash_attn_varlen_func (single kernel)
        if _FLASH_ATTN_AVAILABLE:
            ...
            cu_seqlens = user_offsets.cpu().to(torch.int32)
            max_seqlen = int((user_offsets[1:] - user_offsets[:-1]).max())
        ...
        # Path 2: per-user SDPA with is_causal=True
        outputs = []
    ...
    # Original path: use mask or single-user is_causal
    if hasattr(F, "scaled_dot_product_attention"):
        attn_mask = None
        if not is_causal and extension is not None and "mask" in extension:
    ...
    elif extension is not None and "mask" in extension:
        mask = extension["mask"]
...
class SMoE(nn.Module):
    ...
    def forward(self, x):
        ...
        if V104_BATCHED_SMOE_ENABLED and getattr(self, 'batched_gemm_active', False):
            max_count = int(counts.max().item())
            if max_count > 0:
                padded_x = x_flat.new_zeros(self.num_experts, max_count, D)
                for i in range(self.num_experts):
                    count = int(counts[i].item())
                    if count > 0:
                        start = int(starts[i].item())
                        end = start + count
                        padded_x[i, :count] = x_flat[token_idx_sorted[start:end]]

                # Batched GEMM: [8, max_count, D] @ [8, D, dim_ff] -> [8, max_count, dim_ff]
                hidden = torch.bmm(padded_x, self.fc1_weight_stack.transpose(1, 2)) + self.fc1_bias_stack.unsqueeze(1)
                hidden = F.relu(hidden)
                # [8, max_count, dim_ff] @ [8, dim_ff, D] -> [8, max_count, D]
                expert_outs = torch.bmm(hidden, self.fc2_weight_stack.transpose(1, 2)) + self.fc2_bias_stack.unsqueeze(1)
                ...
                for i in range(self.num_experts):
                    ...
                    if count > 0:
                        ...
                        out_flat.index_add_(0, cur_token_idx, expert_outs[i, :count] * cur_weight)
        else:
            for i in range(self.num_experts):
                count = int(counts[i].item())
                if count == 0:
...
class CTRModel(nn.Module):
    def __init__(self, rep_encoder, seq_encoder, d_model):
        ...
        self.v33_causal_tri_cache = OrderedDict()
        self.v33_causal_tri_cache_bytes = 0
        self.v33_causal_tri_cache_hits = 0
        self.v33_causal_tri_cache_misses = 0

    def get_v33_causal_triangle(self, device, tokens):
        key = (str(device), int(tokens))
        cached = self.v33_causal_tri_cache.get(key)
        if cached is not None:
            self.v33_causal_tri_cache.move_to_end(key)
            self.v33_causal_tri_cache_hits += 1
            return cached

        self.v33_causal_tri_cache_misses += 1
        causal = torch.ones(
            (tokens, tokens),
            device=device,
            dtype=torch.bool,
        ).tril()
        causal_bytes = causal.numel() * causal.element_size()
        if (
            V33_CAUSAL_TRI_CACHE_MAX > 0
            and V33_CAUSAL_TRI_CACHE_MAX_BYTES > 0
            and causal_bytes <= V33_CAUSAL_TRI_CACHE_MAX_BYTES
        ):
            self.v33_causal_tri_cache[key] = causal
            self.v33_causal_tri_cache_bytes += causal_bytes
            self.v33_causal_tri_cache.move_to_end(key)
            while (
                len(self.v33_causal_tri_cache) > V33_CAUSAL_TRI_CACHE_MAX
                or self.v33_causal_tri_cache_bytes > V33_CAUSAL_TRI_CACHE_MAX_BYTES
            ):
                _, evicted = self.v33_causal_tri_cache.popitem(last=False)
                self.v33_causal_tri_cache_bytes -= evicted.numel() * evicted.element_size()
        return causal
    ...
    def get_sequence_causal_mask(self, seq_info):
        ...
        if V33_CAUSAL_TRI_CACHE_ENABLED:
            causal = self.get_v33_causal_triangle(seq_info.device, user_ids.numel())
        else:
            causal = torch.ones(
    ...
    def forward(self, batch):
        ...
        if V103_PER_USER_CAUSAL_SDPA:
            extension = {"is_causal": True, "user_offsets": batch["user_offsets"]}
        else:
        ...
        if getattr(self, '_v105_fp16_final_linear_active', False):
...
def apply_v104_batched_smoe(model):
    """Pre-stack expert weights for batched GEMM in SMoE layers."""
    if not V104_BATCHED_SMOE_ENABLED:
        _info("[INFO] V104 batched SMoE disabled by GRAB_V104_BATCHED_SMOE")
        return 0
    converted = 0
    for moe in model.seq_encoder.moe:
        # Stack expert weights: [num_experts, out_features, in_features]
        fc1_weights = torch.stack([e.fc1.weight.data for e in moe.experts])
        fc1_biases = torch.stack([e.fc1.bias.data for e in moe.experts])
        fc2_weights = torch.stack([e.fc2.weight.data for e in moe.experts])
        fc2_biases = torch.stack([e.fc2.bias.data for e in moe.experts])
        # Store as buffers (not parameters) so they don't appear in state_dict
        moe.register_buffer('fc1_weight_stack', fc1_weights)
        moe.register_buffer('fc1_bias_stack', fc1_biases)
        moe.register_buffer('fc2_weight_stack', fc2_weights)
        moe.register_buffer('fc2_bias_stack', fc2_biases)
        moe.batched_gemm_active = True
        converted += 1
    _info(f"[INFO] V104 batched SMoE enabled: converted {converted} SMoE layers")
    return converted


def apply_v105_int8_weight(model):
    """Weight-only INT8 quantization for Linear layers in seq_encoder.
    
    Stores INT8 weights + per-channel scales as buffers.
    At inference, dequantizes to FP16 before matmul.
    Saves memory bandwidth (INT8 is 2x smaller than FP16).
    """
    if not V105_INT8_WEIGHT:
        _info("[INFO] V105 INT8 weight quantization disabled by GRAB_V105_INT8_WEIGHT")
        return 0
    converted = 0
    for module in model.seq_encoder.modules():
        if isinstance(module, nn.Linear) and module.weight.dtype in (torch.float16, torch.float32):
            ...
            module.register_buffer('_int8_weight', weight_int8)
            module.register_buffer('_int8_scale', scale.squeeze(1).to(torch.float16))
            module._int8_active = True
            converted += 1
    _info(f"[INFO] V105 INT8 weight quantization: converted {converted} Linear layers")
    return converted


def apply_v105_fp16_final_linear(model):
    """Convert the final prediction linear layer to FP16."""
    if not V105_FP16_FINAL_LINEAR:
        _info("[INFO] V105 FP16 final linear disabled by GRAB_V105_FP16_FINAL_LINEAR")
        return 0
    model.linear.half()
    model._v105_fp16_final_linear_active = True
    _info("[INFO] V105 FP16 final linear enabled")
    return 1
...
def _patch_int8_linear(model):
    """Patch Linear.forward to use INT8 weights when available."""
    for module in model.modules():
        if isinstance(module, nn.Linear) and getattr(module, '_int8_active', False):
            original_forward = module.forward
            def make_int8_forward(m):
                def int8_forward(input):
                    weight = m._int8_weight.to(input.dtype) * m._int8_scale.unsqueeze(1)
                    return F.linear(input, weight, m.bias)
                return int8_forward
...
def load_model(ckpt_path=None, device='cuda:0'):
    ...
    apply_v104_batched_smoe(model)
    apply_v105_int8_weight(model)
    apply_v105_fp16_final_linear(model)
    _patch_int8_linear(model)
    model.to(dev)
    model.eval()
...
def main():
    ...
    if batches_cache_dir.exists() and any(batches_cache_dir.glob('shard_*.pt')):
        ...
        test_loader = DataLoader(
            test_dataset,
            batch_size=1 if V105_BATCH_SIZE_1 else 50,
            shuffle=False,
            num_workers=0,
