...
V168_SVD_RANK = int(os.environ.get("GRAB_V168_SVD_RANK", "96"))

# V170: Adaptive rank per layer — each SMoE layer auto-selects its own rank
# based on singular value energy distribution. Layers with fast-decaying spectra
# (concentrated information) get lower rank → faster; layers with slow-decaying
# spectra get higher rank → more accurate. This is "different algorithms for
# different data" — each layer uses its locally optimal rank.
V170_ADAPTIVE_RANK = os.environ.get("GRAB_V170_ADAPTIVE_RANK", "1").lower() not in {"0", "false", "no"}
V170_ENERGY_RATIO = float(os.environ.get("GRAB_V170_ENERGY_RATIO", "0.92"))
V170_MIN_RANK = int(os.environ.get("GRAB_V170_MIN_RANK", "32"))
V170_MAX_RANK = int(os.environ.get("GRAB_V170_MAX_RANK", "96"))
...
V129_LOGIT_BIAS = float(os.environ.get("GRAB_V129_LOGIT_BIAS", str(math.log(1.0 / 1.05913))))
        ...
        if V168_SVD_LOWRANK and not self.training:
            try:
                with torch.no_grad():
                    ...
                    if V170_ADAPTIVE_RANK:
                        # V170: auto-select rank per layer based on singular value energy
                        # Find minimum R that retains V170_ENERGY_RATIO of total energy
                        # Energy = singular value squared; cumsum gives cumulative energy
                        energy1 = S1 ** 2  # [E, K]
                        cum1 = torch.cumsum(energy1, dim=1)  # [E, K]
                        total1 = cum1[:, -1:]  # [E, 1]
                        ratio1 = cum1 / total1  # [E, K]
                        # For each Expert, find min R where ratio >= ENERGY_RATIO
                        mask1 = ratio1 >= V170_ENERGY_RATIO  # [E, K]
                        ranks1 = mask1.int().argmax(dim=1) + 1  # [E]

                        energy2 = S2 ** 2
                        cum2 = torch.cumsum(energy2, dim=1)
                        total2 = cum2[:, -1:]
                        ratio2 = cum2 / total2
                        mask2 = ratio2 >= V170_ENERGY_RATIO
                        ranks2 = mask2.int().argmax(dim=1) + 1

                        # Take max across all Experts (fc1 and fc2) to ensure precision
                        R = int(max(ranks1.max().item(), ranks2.max().item()))
                        R = max(V170_MIN_RANK, min(R, V170_MAX_RANK))
                    else:
                ...
                _info(f"[INFO] V170: SVD adaptive rank R={R} (energy_ratio={V170_ENERGY_RATIO}, fc1: {self._all_fc1_w.shape[1]}x{self._all_fc1_w.shape[2]})")
