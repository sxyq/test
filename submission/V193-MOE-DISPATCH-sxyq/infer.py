...
# V193: PyTorch grouped-dispatch MoE candidate. It preserves TopK routing
# semantics but computes only routed expert tokens instead of dense all-expert
# GEMMs. This is a stepping stone toward fused dispatch/combine + Grouped GEMM.
V193_GROUPED_DISPATCH_MOE = os.environ.get("GRAB_V193_GROUPED_DISPATCH_MOE", "1").lower() not in {"0", "false", "no"}
...
class SMoE(nn.Module):
    ...
    def _v193_grouped_dispatch_forward(self, x, topk_idx, topk_score):
        B, S, D = x.shape
        x_flat = x.reshape(-1, D)
        ns = B * S
        token_idx_flat = torch.arange(ns, device=x.device).unsqueeze(1).expand(-1, self.k).reshape(-1)
        expert_idx_flat = topk_idx.reshape(-1)
        expert_score_flat = topk_score.reshape(-1, 1)
        order = torch.argsort(expert_idx_flat)
        expert_idx_sorted = expert_idx_flat.index_select(0, order)
        token_idx_sorted = token_idx_flat.index_select(0, order)
        expert_score_sorted = expert_score_flat.index_select(0, order)
        counts = torch.bincount(expert_idx_sorted, minlength=self.num_experts)
        starts = torch.cumsum(counts, dim=0) - counts
        out_flat = x_flat.new_zeros(ns, D)
        use_svd = V168_SVD_LOWRANK and hasattr(self, '_v168_fc1_left')
        for expert_id in range(self.num_experts):
            ...
            if use_svd:
                hidden = selected_x.matmul(self._v168_fc1_left[expert_id])
                hidden = hidden.matmul(self._v168_fc1_right[expert_id])
                hidden = hidden + self._all_fc1_b[expert_id]
                hidden = F.relu(hidden)
                expert_out = hidden.matmul(self._v168_fc2_left[expert_id])
                expert_out = expert_out.matmul(self._v168_fc2_right[expert_id])
                expert_out = expert_out + self._all_fc2_b[expert_id]
            else:
                hidden = selected_x.matmul(self._all_fc1_w[expert_id]) + self._all_fc1_b[expert_id]
                hidden = F.relu(hidden)
                expert_out = hidden.matmul(self._all_fc2_w[expert_id]) + self._all_fc2_b[expert_id]
    ...
    def forward(self, x):
        ...
        if V193_GROUPED_DISPATCH_MOE and not self.training and self._v109_dense_weights_ready:
            out = self._v193_grouped_dispatch_forward(x, topk_idx, topk_score)
