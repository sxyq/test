...
# 百度生成式推荐广告排序推理性能优化
# 环境构建脚本
# 注意：此脚本在评测环境中执行，需在720秒内完成（平台硬性限制）
...
# 加 timeout 300 保护：若编译超过5分钟则跳过，避免 build_env.sh 整体超时被杀
export PYTHONPATH="${LIB_DIR}:${PYTHONPATH}"
timeout 300 pip install flash-attn --no-build-isolation --target "${LIB_DIR}" -i https://mirrors.aliyun.com/pypi/simple/ 2>/dev/null || {
    echo "[WARNING] flash-attn 安装失败或超时，将使用 fallback SDPA 路径"
}
