...
V166_TORCH_COMPILE = os.environ.get("GRAB_V166_TORCH_COMPILE", "1").lower() not in {"0", "false", "no"}
...
def load_model(ckpt_path=None, device='cuda:0'):
    ...
    # V166: torch.compile with default mode (operator fusion, no CUDA Graph)
    if V166_TORCH_COMPILE and hasattr(torch, "compile"):
        try:
            model = torch.compile(model, mode="default")
            _info("[INFO] V166: torch.compile enabled (mode=default)")
        except Exception as e:
            _warn(f"[WARNING] V166 torch.compile failed: {e}")
