...
class SMoE(nn.Module):
    ...
    def _v109_prepare_dense_weights(self):
        ...
        # before _v109_prepare_dense_weights runs, so _all_fc1_w is FP16.
        # FP16 SVD on pruned weights (768 vs 1024) can be unstable → PCOC drift.
        if V168_SVD_LOWRANK and not self.training:
            try:
                ...
                with torch.no_grad():
                    ...
                    self._v168_fc1_left = (U1[:, :, :R] * S1[:, :R].unsqueeze(1)).contiguous().half()
                    self._v168_fc1_right = Vh1[:, :R, :].contiguous().half()
                    w2 = self._all_fc2_w.float()
                    U2, S2, Vh2 = torch.linalg.svd(w2, full_matrices=False)
                    self._v168_fc2_left = (U2[:, :, :R] * S2[:, :R].unsqueeze(1)).contiguous().half()
                    self._v168_fc2_right = Vh2[:, :R, :].contiguous().half()
