...
# V190: keep the V188 DataLoader-controlled path, but make the balanced-batch
# execution order explicit and shape-stable. The target is not a new packing
# algorithm; it is to lock batch order to the locally best proxy:
# larger max_len first, then larger token_total, without V189-style median-first
# reshuffling that disturbed adjacent batch shape continuity.
V190_SHAPE_STABLE_BATCH_ORDER = os.environ.get("GRAB_V190_SHAPE_STABLE_BATCH_ORDER", "1").lower() not in {"0", "false", "no"}
V190_PRECOMPUTE_USER_RECORDS = os.environ.get("GRAB_V190_PRECOMPUTE_USER_RECORDS", "1").lower() not in {"0", "false", "no"}
V190_FAST_COLLATE = os.environ.get("GRAB_V190_FAST_COLLATE", "1").lower() not in {"0", "false", "no"}
...
def reorder_balanced_batches(grouped_batches):
    if not (V190_SHAPE_STABLE_BATCH_ORDER and grouped_batches):
        return grouped_batches
    return sorted(
        grouped_batches,
        key=lambda item: (-item[1]["max_len"], -item[1]["token_total"], item[1]["batch_idx"]),
    )
...
class CTRUserDataset(Dataset):
    def __init__(self, item_dict, user_seq=None, max_feasign_per_slot=None, pred_logids=None):
        ...
        self.max_slot_id = 28
        self.max_sign_id = max_sign_id
        self.user_records = {}
        if V190_PRECOMPUTE_USER_RECORDS:
            self._precompute_user_records()
    ...
    def _precompute_user_records(self):
        for userid in self.user_ids:
            ...
            if self.user_seq and userid in self.user_seq:
                ...
                items = sorted(items, key=lambda x: seq_order.get(x[0], x[0]))
            else:
                items = sorted(items, key=lambda x: x[0])
            logids, feasigns, labels, pred_mask = [], [], [], []
            for logid, feasign, label in items:
                logids.append(logid)
                feasigns.append(feasign)
                labels.append(label)
                pred_mask.append(1 if logid in self.pred_logids else 0)
            self.user_records[userid] = {
                'userid': userid,
                'logids': logids,
                'feasigns': feasigns,
                'labels': labels,
    ...
    def __getitem__(self, index):
        ...
        if V190_PRECOMPUTE_USER_RECORDS:
            return self.user_records[userid]
        items = self.user_items[userid]
        if self.user_seq and userid in self.user_seq:
            seq_order = {logid: i for i, logid in enumerate(self.user_seq[userid])}
...
def make_collate_fn(max_slot_id):
    def collate_user_batch(batch):
        ...
        for item in batch:
            logids = item['logids']
            all_userids.extend([item['userid']] * len(logids))
            all_logids.extend(logids)
            all_labels.extend(item['labels'])
            all_pred_masks.extend(item['pred_mask'])
            all_feasigns.extend(item['feasigns'])
            user_offsets.append(len(all_labels))
        slot_data = {}
        if V190_FAST_COLLATE:
            slot_values = [[] for _ in range(max_slot_id + 1)]
            slot_offsets = [[0] for _ in range(max_slot_id + 1)]
            for feasign in all_feasigns:
                for slot in range(1, max_slot_id + 1):
                    slot_offsets[slot].append(slot_offsets[slot][-1])
                for slot, signs in feasign.items():
                    if 1 <= slot <= max_slot_id and signs:
                        slot_values[slot].extend(signs)
                        slot_offsets[slot][-1] += len(signs)
            for slot in range(1, max_slot_id + 1):
                slot_data[slot] = (torch.tensor(slot_values[slot], dtype=torch.long),
                                   torch.tensor(slot_offsets[slot], dtype=torch.long))
        else:
            for slot in range(1, max_slot_id + 1):
                values, offsets = [], [0]
                for feasign in all_feasigns:
