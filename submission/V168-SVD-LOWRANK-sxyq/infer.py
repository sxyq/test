...
class SMoE(nn.Module):
    ...
    def _v109_prepare_dense_weights(self):
        ...
        # Reduces GEMM FLOPs from D*dim_ff to R*(D+dim_ff), ~50-70% reduction for R=64-128
        # Zero overhead after decomposition: just two smaller GEMMs instead of one large
        if V168_SVD_LOWRANK and not self.training:
            try:
                R = min(V168_SVD_RANK, min(self._all_fc1_w.shape[1], self._all_fc1_w.shape[2]))
                with torch.no_grad():
                    # fc1: [E, D, dim_ff] → SVD → U[E,D,K], S[E,K], Vh[E,K,dim_ff] where K=min(D,dim_ff)
                    U1, S1, Vh1 = torch.linalg.svd(self._all_fc1_w, full_matrices=False)
                    # Truncate to rank R: fc1 ≈ (U[:,:,:R] * S[:,:R]) @ Vh[:,:R,:]
                    self._v168_fc1_left = (U1[:, :, :R] * S1[:, :R].unsqueeze(1)).contiguous()   # [E, D, R]
                    self._v168_fc1_right = Vh1[:, :R, :].contiguous()                           # [E, R, dim_ff]

                    # fc2: [E, dim_ff, D] → SVD
                    U2, S2, Vh2 = torch.linalg.svd(self._all_fc2_w, full_matrices=False)
                    self._v168_fc2_left = (U2[:, :, :R] * S2[:, :R].unsqueeze(1)).contiguous()  # [E, dim_ff, R]
                    self._v168_fc2_right = Vh2[:, :R, :].contiguous()                           # [E, R, D]

                _info(f"[INFO] V168: SVD low-rank R={R} (fc1: {self._all_fc1_w.shape[1]}x{self._all_fc1_w.shape[2]} → {self._all_fc1_w.shape[1]}x{R} @ {R}x{self._all_fc1_w.shape[2]})")
            except Exception as e:
                _warn(f"[WARNING] V168 SVD decomposition failed: {e}, using full-rank weights")
    ...
    def forward(self, x):
        ...
        if V109_DENSE_SMOE and not self.training and self._v109_dense_weights_ready:
            ...
            if V168_SVD_LOWRANK and hasattr(self, '_v168_fc1_left'):
                ...
                hidden = torch.matmul(x_flat.unsqueeze(0), self._v168_fc1_left)   # [E, NS, R]
                hidden = torch.matmul(hidden, self._v168_fc1_right)              # [E, NS, dim_ff]
                hidden = hidden + self._all_fc1_b.unsqueeze(1)
                hidden = F.relu(hidden)
                # fc2: [E,NS,dim_ff] @ [E,dim_ff,R] @ [E,R,D] → [E,NS,D]
                out_all = torch.matmul(hidden, self._v168_fc2_left)              # [E, NS, R]
                out_all = torch.matmul(out_all, self._v168_fc2_right)            # [E, NS, D]
                out_all = out_all + self._all_fc2_b.unsqueeze(1)
            else:
                # Original dense path: [1, NS, D] @ [E, D, dim_ff] → [E, NS, dim_ff]
