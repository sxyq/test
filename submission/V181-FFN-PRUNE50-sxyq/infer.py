...
# V153: Structured pruning of SMoE expert hidden dimension (dim_ff 1024→512, 50%)
# Zero overhead: just smaller weight matrices. V154 standalone got AUC=0.75292 (higher than 25%!).
# V181: 50% pruning + SVD + bias=0, based on V177 (best score 69.90).
# V154 verified 50% pruning is compliant (score 68.45, AUC=0.75292, PCOC=1.07224).
V153_PRUNE_FFN = os.environ.get("GRAB_V153_PRUNE_FFN", "1").lower() not in {"0", "false", "no"}
V153_PRUNE_RATIO = float(os.environ.get("GRAB_V153_PRUNE_RATIO", "0.50"))
...
# V177: bias=0 to test if V176's anomaly was PCOC超标 from FFN_prune+SVD interaction
V129_SKIP_LAYERS = os.environ.get("GRAB_V129_SKIP_LAYERS", "")
V129_LOGIT_BIAS = float(os.environ.get("GRAB_V129_LOGIT_BIAS", "0.0"))
